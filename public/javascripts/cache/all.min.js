...
animate(settings.onAfterFirst);delete attr[key];}});animate(settings.onAfter);function animate(callback){$elem.animate(attr,duration,settings.easing,callback&&function(){callback.call(this,target,settings);});};function max(Dim){var attr='scroll'+Dim,doc=elem.ownerDocument;return win?Math.max(doc.documentElement[attr],doc.body[attr]):elem[attr];};}).end();};function both(val){return typeof val=='object'?val:{top:val,left:val};};})(jQuery);jQuery.cookie=function(name,value,options){if(typeof value!='undefined'){options=options||{};if(value===null){value='';options.expires=-1;}
...
return cookieValue;}};(function($){var opt;$.fn.jqprint=function(options,callback){opt=$.extend({},$.fn.jqprint.defaults,options);var $element=(this instanceof jQuery)?this:$(this);if(opt.operaSupport&&$.browser.opera){var tab=window.open("","jqPrint-preview");tab.document.open();var doc=tab.document;}else{var $iframe=$("<iframe  />");if(!opt.debug)
$iframe.css({position:"absolute",width:"0px",height:"0px",left:"-600px",top:"-600px"});$iframe.appendTo("body");var doc=$iframe[0].contentWindow.document;}
if(opt.importCSS){$("link").each(function(){doc.write("<link type='text/css' rel='stylesheet' href='"+$(this).attr("href")+"' media='"+$(this).attr("media")+"' />");});}
if(opt.printContainer){doc.write($element.outer());}
else{$element.each(function(){doc.write($(this).html());});}
doc.close();(opt.operaSupport&&$.browser.opera?tab:$iframe[0].contentWindow).focus();setTimeout(function(){(opt.operaSupport&&$.browser.opera?tab:$iframe[0].contentWindow).print();if(tab)tab.close();if(typeof callback=='function')callback.call(this,$element);},1000);}
$.fn.jqprint.defaults={debug:false,importCSS:true,printContainer:true,operaSupport:true};jQuery.fn.outer=function(){return $($('<div></div>').html(this.clone())).html();}})(jQuery);jQuery.fn.sortElements=(function(){var sort=[].sort;return function(comparator,getSortable){getSortable=getSortable||function(){return this;};var placements=this.map(function(){var sortElement=getSortable.call(this),parentNode=sortElement.parentNode,nextSibling=parentNode.insertBefore(document.createTextNode(''),sortElement.nextSibling);return function(){if(parentNode===this){throw new Error("You can't sort elements if any one is a descendant of another.");}
parentNode.insertBefore(this,nextSibling);parentNode.removeChild(nextSibling);};});return sort.call(this,comparator).each(function(i){placements[i].call(getSortable.call(this));});};})();jQuery.ajaxSetup({beforeSend:function(xhr){xhr.setRequestHeader("Accept","text/javascript")}});jQuery.fn.hinty=function(){return this.each(function(){var $this=jQuery(this);var title=this.title;if(!$this.val()||$this.val()==''||$this.val()==title){$this.addClass('hint_text');$this.val(title);}
$this.focus(setCaretPos).click(setCaretPos);function setCaretPos(){if(this.value==title){if(this.setSelectionRange){this.focus();if($this.parent().is(':visible')&&$this.is(':visible')){this.setSelectionRange(0,0);}}else if(this.createTextRange){var range=this.createTextRange();range.collapse(true);range.moveEnd('character',0);range.moveStart('character',0);range.select();}}}
$this.keydown(function(){if($this.val()==title){$this.removeClass('hint_text');$this.val('');}});$this.blur(function(){if($this.val()==''){$this.addClass('hint_text');$this.val(title);}});});}
jQuery.fn.formBouncer=function(){return this.each(function(){jQuery(this).live('submit',function(){$('.invalid',this).removeClass('invalid');$('.error',this).remove();return $(this).runValidation().data('valid');});});}
jQuery.fn.runValidation=function(silent){var valid_email=/^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/,valid_phone=/^\(?(\d{3})\)?[- ]?(\d{3})[- ]?(\d{4})$/,valid_date=/^(\d{1,2})[./-](\d{1,2})[./-](\d{4})$/,valid_zip=/\d{5}/,numeric_class_regex=/(numeric_)/,form=$(this),silent=silent||form.hasClass('silent');errors='';function valid_credit_card(value){if(/[^0-9-\s]+/.test(value))return false;var nCheck=0,nDigit=0,bEven=false;value=value.replace(/\D/g,"");for(n=value.length-1;n>=0;n--){var cDigit=value.charAt(n),nDigit=parseInt(cDigit,10);if(bEven){if((nDigit*=2)>9)
nDigit-=9;}
nCheck+=nDigit;bEven=!bEven;}
return(nCheck%10)==0;}
function password_input_exists(form){return jQuery('input[type=password]',form).length>0}
function error_html(input,msg){if(form.hasClass('less_verbose')){return'<p>'+msg+'</p>';}else{var name=input.attr('name').split('[');name=name[name.length-1].replace(']','').replaceAll('_',' ');return'<p>'+capitalize(name)+' '+msg+'.</p>';}}
function markInvalid(input,form){if(!input.hasClass('invalid'))input.addClass('invalid').removeClass('hint_text');$('.invalid',form).eq(0).focus();$('.invalid',form).blur(function(){$(this).parent().runValidation(form.hasClass('silent'));});}
function is_numeric(input){return numeric_class_regex.test(input.attr('class'));}
$('.error',form).remove();jQuery('input, select, textarea',form).each(function(){var input=jQuery(this).removeClass('invalid'),error='';if(!input.attr('disabled')){if(input.hasClass('required')){if(input.is(':checkbox')&&!input.is(':checked')){error=error_html(input,'must be checked');markInvalid(input,form);}else if(input.is(':radio')&&!$('input[name='+input.attr('name')+']:checked').length){error='<p>pick one</p>';markInvalid(input,form);}else if(input.val()==''||input.val()==input.attr('title')){error=error_html(input,'is required');markInvalid(input,form);}
errors+=error;}
if(input.hasClass('email')&&(input.val()!=''&&valid_email.test(input.val())==false)){error=error_html(input,'is not valid');errors+=error;markInvalid(input,form);}
if(is_numeric(input)){if(input.hasClass('numeric_phone')&&!valid_phone.test(input.val())){error=error_html(input,'must be a valid US phone number w/ area code');markInvalid(input,form);}else if(input.hasClass('numeric_date')&&!valid_date.test(input.val())){error=error_html(input,'must be a valid date: mm/dd/yyyy');markInvalid(input,form);}else if(input.hasClass('numeric_zip')&&!valid_zip.test(input.val())){error=error_html(input,'must be a 5 digit zip');markInvalid(input,form);}
errors+=error;}
if(input.hasClass('confirm')&&password_input_exists(form)&&jQuery('input[type=password]',form)[0].value!=input.val()){error='<p>Passwords do not match.</p>';errors+=error;markInvalid(input,form);}
if(input.hasClass('credit_card')&&!valid_credit_card(input.val())){error=error_html(input,'is invalid');errors+=error;markInvalid(input,form);}}
if(error!=''&&!silent){jQuery('.error',input.parent()).remove();input.before('<div class=\'flash error hidden\'>'+error+'</div>');jQuery('.error',input.parent()).slideDown();}});errors!=''?form.data('valid',false):form.data('valid',true);return form;}
var Inflector=function(){};Inflector.prototype={plural:[[/(quiz)$/i,"$1zes"],[/^(ox)$/i,"$1en"],[/([m|l])ouse$/i,"$1ice"],[/(matr|vert|ind)ix|ex$/i,"$1ices"],[/(x|ch|ss|sh)$/i,"$1es"],[/([^aeiouy]|qu)y$/i,"$1ies"],[/(hive)$/i,"$1s"],[/(?:([^f])fe|([lr])f)$/i,"$1$2ves"],[/sis$/i,"ses"],[/([ti])um$/i,"$1a"],[/(buffal|tomat)o$/i,"$1oes"],[/(bu)s$/i,"$1ses"],[/(alias|status)$/i,"$1es"],[/(octop|vir)us$/i,"$1i"],[/(ax|test)is$/i,"$1es"],[/s$/i,"s"],[/$/,"s"]],singular:[[/(quiz)zes$/i,"$1"],[/(matr)ices$/i,"$1ix"],[/(vert|ind)ices$/i,"$1ex"],[/^(ox)en/i,"$1"],[/(alias|status)es$/i,"$1"],[/(octop|vir)i$/i,"$1us"],[/(cris|ax|test)es$/i,"$1is"],[/(shoe)s$/i,"$1"],[/(o)es$/i,"$1"],[/(bus)es$/i,"$1"],[/([m|l])ice$/i,"$1ouse"],[/(x|ch|ss|sh)es$/i,"$1"],[/(m)ovies$/i,"$1ovie"],[/(s)eries$/i,"$1eries"],[/([^aeiouy]|qu)ies$/i,"$1y"],[/([lr])ves$/i,"$1f"],[/(tive)s$/i,"$1"],[/(hive)s$/i,"$1"],[/([^f])ves$/i,"$1fe"],[/(^analy)ses$/i,"$1sis"],[/((a)naly|(b)a|(d)iagno|(p)arenthe|(p)rogno|(s)ynop|(t)he)ses$/i,"$1$2sis"],[/([ti])a$/i,"$1um"],[/(n)ews$/i,"$1ews"],[/s$/i,""]],irregular:[['move','moves'],['sex','sexes'],['child','children'],['man','men'],['person','people']],uncountable:["sheep","fish","series","species","money","rice","information","equipment"],initialize:function(){},ordinalize:function(number){if(11<=parseInt(number)%100&&parseInt(number)%100<=13){return number+"th";}else{switch(parseInt(number)%10){case 1:return number+"st";case 2:return number+"nd";case 3:return number+"rd";default:return number+"th";}}},pluralize:function(word){for(var i=0;i<this.uncountable.length;i++){var uncountable=this.uncountable[i];if(word.toLowerCase()==uncountable){return uncountable;}}
for(var i=0;i<this.irregular.length;i++){var singular=this.irregular[i][0];var plural=this.irregular[i][1];if((word.toLowerCase()==singular)||(word==plural)){return plural;}}
for(var i=0;i<this.plural.length;i++){var regex=this.plural[i][0];var replace_string=this.plural[i][1];if(regex.test(word)){return word.replace(regex,replace_string);}}},singularize:function(word){for(var i=0;i<this.uncountable.length;i++){var uncountable=this.uncountable[i];if(word.toLowerCase()==uncountable){return uncountable;}}
for(var i=0;i<this.irregular.length;i++){var singular=this.irregular[i][0];var plural=this.irregular[i][1];if((word.toLowerCase()==singular)||(word==plural)){return singular;}}
for(var i=0;i<this.singular.length;i++){var regex=this.singular[i][0];var replace_string=this.singular[i][1];if(regex.test(word)){return word.replace(regex,replace_string);}}}}
function ordinalize(number){var i=new Inflector;return i.ordinalize(number);}
function pluralize(){var i=new Inflector;var count=arguments[0];var singular=arguments[1];var plural=arguments[2];if(arguments.length<2)return"";if(isNaN(count))return"";return count+" "+(1==parseInt(count)?singular:plural||i.pluralize(singular));}
function singularize(plural){var i=new Inflector;return i.singularize(plural);}
(function($){var timer;$.fn.src=function(url,onLoad,options){setIFrames($(this),onLoad,options,function(){this.src=url;});return $(this);}
$.fn.squirt=function(content,onLoad,options){setIFrames($(this),onLoad,options,function(){var doc=this.contentDocument||this.contentWindow.document;doc.open();doc.writeln(content);doc.close();});return this;}
function setIFrames(iframes,onLoad,options,iFrameSetter){iframes.each(function(){if(this.tagName=="IFRAME")setIFrame(this,onLoad,options,iFrameSetter);});}
function setIFrame(iframe,onLoad,options,iFrameSetter){var iframe;iframe.onload=null;if(timer)clearTimeout(timer);var defaults={timeoutDuration:0,timeout:null,}
...
return iframe;};})(jQuery);(function(k,s){if(!k.jqDock){var r=["Top","Right","Bottom","Left"],E=["Major","Minor"],y=["mouseenter","mousemove","mouseleave"],z=['<div style="position:relative;padding:0;',"margin:0;border:0 none;background-color:transparent;",'">'],p={v:{wh:"height",xy:1,tl:"top",lead:0,trail:2,inv:"h"},h:{wh:"width",xy:0,tl:"left",lead:3,trail:1,inv:"v"}},n=[],F=[0,0],S=function(){},w=function(a){a=parseInt(a,10);return isNaN(a)?0:a},t=function(a,d){if(a[d]){s.clearTimeout(a[d]);a[d]=null}},G=function(a){var d=p[a.Opts.vh],b=0;if(a=a.Elem[a.Current]){b=a.Pad[d.lead]+a.Pad[d.trail];b=Math.floor((F[d.xy]-a.Wrap.parent().offset()[d.tl])*(b+a.Initial)/(b+a.Major))+a.Offset}return b},H=function(a){return a?1*((a.id||"").match(/^jqDock(\d+)$/)||[0,-1])[1]:-1},I=function(a){for(var d=true,b=n.length,e;d&&b--;)for(e=n[b].Elem.length;d&&e--;)d=n[b].Elem[e].Img[0]!==a;return d?!d:n[b].Elem[e]},J=function(a){var d=n[a.data.id],b=d.Elem[a.data.idx];b.height=this.height;b.width=this.width;++d.Loaded>=d.Elem.length&&s.setTimeout(function(){k.jqDock.initDock(a.data.id)},0)},T=function(a,d){for(var b;a&&a.ownerDocument&&a!==d;){if(b=a.className.toString().match(/jqDockMouse(\d+)/))return 1*b[1];a=a.parentNode}return-1},K=function(a,d,b){var e={},c=p[b].wh;b=p[p[b].inv].wh;e[c]=d;e[b]=Math.round(d*a[b]/a[c]);return e},U=function(){k(this).prev("img").trigger("click");return false},u=function(a,d){var b=a.Elem[a.Current];if(b&&a.Opts.labels)b.Label.el[b.Label.txt&&d?"show":"hide"]()},A=function(a,d){var b=a.Opts.labels,e=p[a.Opts.vh],c=a.Elem[a.Current],f,j,h,i;if(c&&b){h=c.Label;i=h.el;if(h.mc){h.mc=0;for(f in p){h[f]=i[p[f].wh]();for(j in{lead:1,trail:1})h[f]+=w(i.css("padding"+r[p[f][j]]))}}b.charAt(0)=="m"&&i.css({top:Math.floor((c[E[p[e.inv].xy]]-h.v)/2)});b.charAt(1)=="c"&&i.css({left:Math.floor((c[E[e.xy]]-h.h)/2)})}d&&u(a,1)},L=function(a){for(var d=a.childNodes.length,b;d;){b=a.childNodes[--d];if(b.childNodes&&b.childNodes.length)L(b);else b.nodeType==3&&a.removeChild(b)}},M=function(a){a.Menu.css({visibility:"visible"}).show()},B=function(a){var d=a.Opts.idle;if(d){t(a,"Idler");a.Idler=s.setTimeout(function(){a.Menu.trigger("dockidle")},d)}},V=function(a,d,b){var e=a.Opts,c=e.labels,f=d.Label;f.txt=e.setLabel.call(a.Menu[0],d.Title,b);f.mc=f.mc&&!!f.txt;f.el=k('<div class="jqDockLabel jqDockLabel'+d.Link+'" style="position:absolute;margin:0;"><div class="jqDockLabelText">'+f.txt+"</div></div>").hide().insertAfter(d.Img);if(c&&f.txt){a=c.charAt(0)=="b";c=c.charAt(1)=="r";f.el.css({top:a?"auto":0,left:c?"auto":0,bottom:a?0:"auto",right:c?0:"auto"}).click(U)}},x=function(a,d){var b=n[a],e=b.Opts,c=p[e.vh].wh,f=b.Elem.length,j,h;for(d=d||G(b);f;){j=b.Elem[--f];h=Math.abs(d-j.Centre);j.Final=h<e.distance?j[c]-Math.floor((j[c]-j.Initial)*Math.pow(h,e.coefficient)/e.attenuation):j.Initial}},W=function(a){return a},N=function(a,d,b,e){a=n[a];var c=a.Elem[d],f=a.Opts,j=a.Yard,h=p[f.vh],i=p[h.inv],l=c.src!=c.altsrc,o,g,m;if(e||c.Major!=b){o=k.boxModel||f.vh=="v"?0:a.Border[h.lead]+a.Border[h.trail];if(l&&!e&&c.Major==c.Initial)c.Img[0].src=c.altsrc;a.Spread+=b-c.Major;g=K(c,b,f.vh);m=f.size-g[i.wh];switch(f.align){case"top":case"left":g["margin"+r[i.trail]]=m;break;case"middle":case"center":g["margin"+r[i.lead]]=(m+m%2)/2;g["margin"+r[i.trail]]=(m-m%2)/2;break;default:g["margin"+r[i.lead]]=m}if(b!=c.Major||e&&!d){f.flow&&j.parent()[h.wh](a.Spread+a.Border[h.lead]+a.Border[h.trail]);j[h.wh](a.Spread+o)}c.Wrap.css(g);f.flow||j.css(h.tl,Math.floor(Math.max(0,(a[h.wh]-a.Spread)/2)));a.OnDock&&A(a);c.Major=b;c.Minor=g[i.wh];if(l&&!e&&b==c.Initial)c.Img[0].src=c.src}},C=function(a,d){var b=n[a],e=b.Opts,c=p[e.vh],f=e.duration+e.step,j=0,h;if(b.Stamp){f=(new Date).getTime()-b.Stamp;if(f>=e.duration)b.Stamp=0}if(f>e.step){for(e=f<e.duration?f/e.duration:0;j<b.Elem.length;){f=b.Elem[j];h=(f.Final-f.Initial)*e;h=d?e?Math.floor(f.Final-h):f.Initial:e?Math.floor(f.Initial+h):f.Final;N(a,j++,h)}if(b.Spread>b[c.wh]){b.Yard.parent()[c.wh](b.Spread+b.Border[c.lead]+b.Border[c.trail]);b[c.wh]=b.Spread}}},O=function(a,d){var b=n[a],e=b.Elem,c=e.length;if(!b.OnDock){for(;c--&&e[c].Major<=e[c].Initial;);G(b);if(c<0){for(c=e.length;c--;)e[c].Major=e[c].Final=e[c].Initial;b.Current=-1;d||B(b)}else{C(a,true);s.setTimeout(function(){O(a,d)},b.Opts.step)}}},P=function(a){var d=n[a],b=d.Elem,e=b.length;if(d.OnDock){for(;e--&&b[e].Major>=b[e].Final;);if(e<0){d.Xpand=1;u(d,1)}else{x(a);C(a);s.setTimeout(function(){P(a)},d.Opts.step)}}},D=function(a,d,b,e){var c=n[d],f=c.Elem,j=f.length;switch(a){case 0:c.OnDock=1;c.Current>=0&&c.Current!==b&&u(c);c.Current=b;A(c,c.Xpand);c.Stamp=(new Date).getTime();x(d);P(d);break;case 1:if(b!==c.Current){u(c);c.Current=b}A(c,c.Xpand);if(c.OnDock&&c.Xpand){x(d);C(d)}break;case 2:t(c,"Inactive");c.OnDock=c.Xpand=0;u(c);for(c.Stamp=(new Date).getTime();j--;)f[j].Final=f[j].Major;O(d,!!e);break;default:}},Q=function(a){var d=H(this),b=n[d],e=b?T(a.target,this):-1,c=-1,f;if(b)if(b.Asleep)b.Sleeper={target:a.target,type:a.type,pageX:a.pageX,pageY:a.pageY};else{f=b.OnDock;t(b,"Idler");F=[a.pageX,a.pageY];if(a.type==y[2])if(f)c=2;else B(b);else{if(b.Opts.inactivity){t(b,"Inactive");b.Inactive=s.setTimeout(function(){D(2,d,e,true)},b.Opts.inactivity)}if(a.type==y[1])if(e<0){if(f&&b.Current>=0)c=2}else c=!f||b.Current<0?0:1;else if(e>=0&&!f)c=0}b.Sleeper=null;c>=0&&D(c,d,e)}},R=function(a){var d=k(".jqDock",this).get(0),b=H(d),e=n[b];if(e)if(a.type=="docknudge"){if(e.Asleep&&!(e.Asleep=e.Opts.onWake.call(this)===false))k(this).trigger("dockwake");if(!e.Asleep){B(e);e.Sleeper&&Q.call(d,e.Sleeper)}}else if(!e.Asleep){t(e,"Idler");if(e.Asleep=e.Opts.onSleep.call(e.Menu[0])!==false){e.Menu.trigger("docksleep");D(2,b,0,true)}}};k.jqDock=function(){return{version:1.5,defaults:{size:48,distance:72,coefficient:1.5,duration:300,align:"bottom",labels:0,source:0,loader:0,inactivity:0,fadeIn:0,fadeLayer:"",step:50,setLabel:0,flow:0,idle:0,onReady:0,onSleep:0,onWake:0},useJqLoader:k.browser.opera||k.browser.safari,initDock:function(a){var d=n[a],b=d.Opts,e=p[b.vh],c=p[e.inv],f=d.Border,j=d.Elem.length,h=z.join(""),i=0,l=0,o,g,m,v=b.fadeLayer;L(d.Menu[0]);for(d.Menu.children().each(function(q,X){var Y=d.Elem[q].Wrap=k(X).wrap(h+h+"</div></div>").parent();b.vh=="h"&&Y.parent().css("float","left")}).find("img").andSelf().css({position:"relative",padding:0,margin:0,borderWidth:0,borderStyle:"none",verticalAlign:"top",display:"block",width:"100%",height:"100%"});l<j;){g=d.Elem[l++];m=K(g,b.size,e.inv);g.Major=g.Final=g.Initial=m[e.wh];g.Wrap.css(m);g.Img.attr({alt:""}).parent("a").andSelf().removeAttr("title");d[c.wh]=Math.max(d[c.wh],b.size+g.Pad[c.lead]+g.Pad[c.trail]);g.Offset=i;g.Centre=i+g.Pad[e.lead]+g.Initial/2;i+=g.Initial+g.Pad[e.lead]+g.Pad[e.trail]}for(l=0;l<j;){g=d.Elem[l++];m=g.Pad[e.lead]+g.Pad[e.trail];d.Spread+=g.Initial+m;for(o in{Centre:1,Offset:1}){x(a,g[o]);i=0;for(c=j;c--;)i+=d.Elem[c].Final+m;if(i>d[e.wh])d[e.wh]=i}}for(;l;){g=d.Elem[--l];g.Final=g.Initial}g=[z[0],z[2],'<div id="jqDock',a,'" class="jqDock" style="position:absolute;top:0;left:0;padding:0;margin:0;overflow:visible;height:',d.height,"px;width:",d.width,'px;"></div></div>'].join("");d.Yard=k("div.jqDock",d.Menu.wrapInner(g));for(c=4;c--;)f[c]=w(d.Yard.css("border"+r[c]+"Width"));for(d.Yard.parent().addClass("jqDockWrap").width(d.width+f[1]+f[3]).height(d.height+f[0]+f[2]);l<j;l++){g=d.Elem[l];m=g.Wrap.parent();for(c=4;c--;)g.Pad[c]&&m.css("padding"+r[c],g.Pad[c]);N(a,l,g.Final,true);m.add(g.Img).addClass("jqDockMouse"+l);V(d,g,l)}g=d.Menu.bind("docknudge dockidle",R);d.Yard.bind(y.join(" "),Q).find("*").css({filter:"inherit"});if(!(d.Asleep=b.onReady.call(d.Menu[0])===false)){a=function(q){q||k(".jqDockFilter",this).add(this).css({filter:""}).removeClass("jqDockFilter");d.Sleep=false;d.Menu.trigger("dockshow").trigger("docknudge")};if(v){if(v!="menu"){g=d.Yard;if(v=="wrap")g=g.parent()}d.Asleep=!!k(".jqDock,.jqDockWrap",g).addClass("jqDockFilter").css({filter:"inherit"});g.css({opacity:0});M(d);g.animate({opacity:1},b.fadeIn,a)}else{M(d);a(1)}}}}}();k.fn.jqDock=function(a){if(this.length&&!this.not("img").length){if(a==="get"){var d=I(this.get(0));return d?k.extend(true,{},d,{Img:null}):null}this.each(function(b,e){var c=I(e),f=0,j,h,i;a=a||{};if(c){j=c.Major==c.Initial;for(i in{src:1,altsrc:1})if(a[i]){h=(k.isFunction(a[i])?a[i].call(e,c[i],i):a[i]).toString();if(c[i]!==h){c[i]=h;f=(i=="src"?j:!j)?i:f}}f&&k(e).attr("src",c[f])}})}else a==="nudge"||a==="idle"?this.filter(".jqDocked").each(function(){R.call(this,{type:"dock"+a})}):this.not(".jqDocked").filter(function(){return!k(this).parents(".jqDocked").length&&!k(this).children().not("img").filter(function(){return k(this).filter("a").children("img").parent().children().length!==1}).length}).addClass("jqDocked").each(function(){var b=k(this),e=n.length,c,f,j,h;n[e]={Elem:[],Menu:b,OnDock:0,Xpand:0,Stamp:0,width:0,height:0,Spread:0,Border:[],Opts:k.extend({},k.jqDock.defaults,a||{},k.metadata?b.metadata():{}),Current:-1,Loaded:0};c=n[e];f=c.Opts;j=!f.loader&&k.jqDock.useJqLoader||f.loader==="jquery";for(h in{size:1,distance:1,duration:1,inactivity:1,fadeIn:1,step:1,idle:1})f[h]=w(f[h]);h=f.coefficient*1;f.coefficient=isNaN(h)?1.5:h;f.labels=/^[tmb][lcr]$/.test(f.labels.toString())?f.labels:f.labels?{top:"br",left:"tr"}[f.align]||"tl":"";f.setLabel=f.setLabel?f.setLabel:W;f.fadeLayer=f.fadeIn?{dock:1,wrap:1}[f.fadeLayer]?f.fadeLayer:"menu":"";for(h in{onSleep:1,onWake:1,onReady:1})f[h]||(f[h]=S);f.attenuation=Math.pow(f.distance,f.coefficient);f.vh={left:1,center:1,right:1}[f.align]?"v":"h";k("img",b).each(function(i,l){var o=k(l),g=o.attr("src"),m=o.parent("a"),v=c.Elem,q;if(!(q=f.source?f.source.call(l,i):""))q=((q=k(l).attr("alt"))&&/\.(gif|jpg|jpeg|png)$/i.test(q)?q:false)||g;v[i]={Img:o,src:g,altsrc:q,Title:o.attr("title")||m.attr("title")||"",Label:{mc:/[mc]/.test(f.labels)},Pad:[],Link:m.length?"Link":"Image"};for(g=4;g--;)c.Elem[i].Pad[g]=w(o.css("padding"+r[g]))});k.each(c.Elem,function(i,l){var o,g=l.altsrc;if(j)k("<img ></j>").bind("load",{id:e,idx:i},J).attr({src:g});else{o=new Image;o.onload=function(){J.call(this,{data:{id:e,idx:i}});o.onload="";o=null};o.src=g}})});return this}}})(jQuery,window);(function($){$.search=function(elements,search,filter,lookup){search=$.trim(search)
...
try{if(sheets[sheetIndex].cssRules||sheets[sheetIndex].rules){thisSheetRules=(sheets[sheetIndex].cssRules)?sheets[sheetIndex].cssRules:sheets[sheetIndex].rules;var ruleIndex=thisSheetRules.length;while(ruleIndex--){if(thisSheetRules[ruleIndex].style&&thisSheetRules[ruleIndex].style.cssText){var text=thisSheetRules[ruleIndex].style.cssText;if(text.toLowerCase().indexOf('url')!=-1){cssPile+=text;}}else if(thisSheetRules[ruleIndex].styleSheet){imported.push(thisSheetRules[ruleIndex].styleSheet);w3cImport=true;}}}}catch(e){}
...
parseCSS(document.styleSheets);return imgUrls;};var browserSniff={code_name:function(){return navigator.appCodeName;},browser_name:function(){return navigator.appName;},app_version:function(){return navigator.appVersion;},user_agent:function(){return navigator.userAgent;},version_major:function(){return parseInt(navigator.appVersion,10);},version_minor:function(){var app_ver=navigator.appVersion,version_major=parseInt(app_ver,10),pos,version_minor=0;if((pos=app_ver.indexOf("MSIE"))!=-1)
version_minor=parseFloat(app_ver.substring(pos+5,app_ver.length));else if(navigator.appName=="Netscape"&&(version_major==3||version_major==4))
version_minor=parseFloat(app_ver);return version_minor;},java_enabled:function(){if(this.browser_name()=="Microsoft"&&verMajor==4&&navigator.javaEnabled())
return"Yes (sort of ..)";else if(navigator.javaEnabled())
return"Yes";else
return"No";},screen_width:function(){if(window.screen)
return screen.width;else if(navigator.javaEnabled()){var toolkit=java.awt.Toolkit.getDefaultToolkit(),screen_size=toolkit.getScreenSize();return screen_size.width;}
return 0;},screen_height:function(){if(window.screen)
return screen.height;else if(navigator.javaEnabled()){var toolkit=java.awt.Toolkit.getDefaultToolkit(),screen_size=toolkit.getScreenSize();return screen_size.height;}
return 0;},color_depth:function(){var color_depth,bits=0;if(window.screen){bits=screen.colorDepth;bits=((bits==14||bits==18)&&bname=="Netscape")?bits-10:bits;color_depth=bits+" bits per pixel";}else color_depth="Only available on browsers v4 or greater";if(bits==4)
color_depth+=" (16 colors)";else if(bits==8)
color_depth+=" (256 colors)";else if(bits==16)
color_depth+=" (65,536 colors -- High Color)";else if(bits==24)
color_depth+=" (16,777,216 colors -- True Color)";else if(bits==32)
color_depth+=" (16,777,216 colors -- True Color [_not_ 4,294,967,296 colors!])";return color_depth;},inner_width:function(){if(document.all)
return document.body.clientWidth;else if(document.layers)
return window.innerWidth;else
return 0;},inner_height:function(){if(document.all)
return document.body.clientHeight;else if(document.layers)
return window.innerHeight;else
return 0;},window_left:function(){if(document.all)
return document.body.scrollLeft;else if(document.layers)
return pageXOffset;else
return 0;},window_top:function(){if(document.all)
return document.body.scrollTop;else if(document.layers)
return pageYOffset;else
return 0;},lang:function(){if(typeof(navigator.userLanguage)=="string")
return navigator.userLanguage;else if(typeof(navigator.language)=="string")
return navigator.language;return'';},platform:function(){if(typeof(navigator.platform)=="string")
return navigator.platform;return'';},referrer:function(){if(self==top)
return document.referrer;else
return parent.document.referrer;},doc_name:function(){return document.URL;},lastmod:function(){return document.lastModified;},os:function(){if(navigator.userAgent.indexOf("Unix")!=-1)
return"Unix";else if(navigator.userAgent.indexOf("Linux")!=-1)
return"Linux";else if(navigator.userAgent.indexOf("NT")!=-1)
return"Windows NT";else if(navigator.userAgent.indexOf("95")!=-1)
return"Windows 95";else if(navigator.userAgent.indexOf("16")!=-1)
return"Windows v3.1x";else if(navigator.userAgent.indexOf("Win")!=-1)
return"Windows v3.1 or NT";else if(navigator.userAgent.indexOf("PPC")!=-1)
return"Macintosh Power PC";else if(navigator.userAgent.indexOf("Mac")!=-1)
return"Macintosh";else
return"Not Detected";}}
$.browserVars=function(){var out='';for(key in browserSniff)
if(browserSniff.hasOwnProperty(key))
out+=key+': '+browserSniff[key]()+"\n";return out;}
$(function(){$('body').addClass('js');$('.hide_if_js').hide();$('.flash').hide().slideDown();try{$.bindPlugins();$.enableEditor();}catch(e){};$('.disabler','.disabled').disabler();$('.anchorListener').anchorDispatch();$('.row_checkable').rowCheckable();$('.pane_switch').paneSwitcher();$('.toggle_div').toggleDiv();$('.trans2opaq').animOpacity();$('.link_div').linkDiv();$('.param_filled').fillWithParam();$('.table_sort').appendParamAndGo();$('.openDiv').openDiv();$.liveSubmit('.search-btn','.search-button','.submit_btn');$('h4 a','#info-accordion').accordion();$('.tabular_content').tabular_content();$('.clickerd').clickOnLoad();$('.numeric_phone').formatPhoneNum();$('.tip_trigger').tooltip();$('.txt_ldr').txt_loader();$('.shimmy').shimmy();$('.focus_onload').eq(0).focus();$('.click_sel').live('focus',function(){$(this).select()});$('#auth_yourself').hide();if($.preloadCssImages)$.preloadCssImages();$.updateUserStat();$('.greyConfirm').live('click',function(){$.greyConfirm('Are you sure?',function(){return true;},function(){return false;});});$.toggleAction(window.location.href,true);$('.sortable','.admin').sortable({opacity:0.3,update:function(e,ui){$.updateModels(e,ui)}});$('.block_sortable','.admin').sortable({opacity:0.3,placeholder:'ui-state-highlight',helper:'clone',update:function(e,ui){$.updateModels(e,ui)}});if($('.mini_calendar').length>0){$('.mini_calendar').datepicker();$('.datepicker_wrap').live('click',function(){$('.mini_calendar',this).focus();});}
$('ul li a','#ov-reports-cnt').click(function(){get_pop_up(this);return false;});$('a','#admin-box').click(function(){var $this=$(this),other_links=$('a:not(this)','#admin-box').removeClass('active');$this.addClass('active');});$('.selectable').live('click',function(){var $this=$(this),checkbox=$('input[type=checkbox]',$this);if(!$this.data('selected')){$this.data('selected',true).addClass('selected');checkbox.trigger('change').attr('checked',true);}else{$this.data('selected',false).removeClass('selected');checkbox.trigger('change').attr('checked',false);}}).siblings('label').live('click',function(){var $this=$(this);$this.click(function(){$this.siblings('.selectable').eq(0).click();});});$('#siteseal').live('click',function(){var godaddy_url='https://seal.godaddy.com:443/verifySeal?sealID=XHjJD1MWNJ2lR4Dt0enfWq2PGeF713whHBQcuu37sFaJRUSR37baz';if($.on_page([['new','rentals']])){window.open(godaddy_url,'SealVerfication','location=yes,status=yes,resizable=yes,scrollbars=no,width=592,height=740');}else{$('<div id="pop_up"><iframe id="site_seal_frame" src="'+godaddy_url+'"></iframe></div>').dialog(default_pop_up_options({title:'Secure Site by GoDaddy.com',width:593,height:853}));}});var radios=$('.radio_wrap').each(function(){var $this=$(this);if($this.children('.radio_select').children(':radio').is(':checked'))
$this.addClass('selected').find('.radio_select').find(':radio').attr('checked',true);});radios.live('click',function(){var $this=$(this);$this.siblings().removeClass('selected').find('.radio_select').find(':radio').attr('checked',false);$this.addClass('selected').find('.radio_select').find(':radio').attr('checked',true);});if($.on_page([['index','admin']])){var admin_links=$('a','#controller_list'),ajax_wrap=$('#ajax_wrap');admin_links.live('click',function(){$.cookie('active_admin_link',this.id)});$('a:not(.ajax_action, .toggle_action, .partial_addable, .add_link, .cancel_link, .click_thru, .ps, .btn)','#admin_panel').live('click',function(){var $this=$(this);if($this.hasClass('admin_link')){admin_links.removeClass('active');$this.addClass('active');}
ajax_wrap.children().fadeTo('fast',0.2);ajax_wrap.addClass('loading').load($this.attr('href')+' #ajax_wrap_inner',function(response,status){if(status=='success'){$.bindPlugins();$.enableEditor();$('.disabler','#ajax_wrap_inner').disabler();fillInFormFieldSelectLists($('#form_controller','#FormsForm').val());}
else $('#ajax_wrap_inner').html(response);$(this).removeClass('loading').children().hide().fadeIn('fast');});return false;});$('form','#ajax_wrap').live('submit',function(){var form=$(this);ajax_wrap.children().fadeTo('fast',0.2);ajax_wrap.addClass('loading');form.ajaxSubmit({target:'#ajax_wrap',success:function(response,status){if(status=='success')setTimeout(function(){$('.flash','#ajax_wrap_inner').slideUpRemove('slow');},7000);ajax_wrap.removeClass('loading').children().hide().fadeIn('fast');}});return false;});$('#client_verify','#ov-member').live('click',function(){var $this=$(this),ajax_loader=$.new_ajax_loader('after',this);if(!$this.data('sending')){ajax_loader.show();$this.data('sending',true);$.post(this.href,{authenticity_token:$.get_auth_token()},function(response){$.with_json(response,function(data){$this.fadeOutRemove('fast');},function(){$this.data('sending',false);});ajax_loader.fadeOutRemove('fast');},'json');}
return false;});if($.cookie('active_admin_link'))$('#'+$.cookie('active_admin_link')).click();}
$('.unique_checkbox').live('click',function(){var $this=$(this);$('input[type=checkbox]',$this.parent().parent().parent()).attr('checked',false);$this.attr('checked',!$this.is(':checked'))});$('.unique_checkbox').live('click',function(){var $this=$(this);$('input[type=checkbox]',$this.parent().parent().parent()).attr('checked',false);$this.attr('checked',!$this.is(':checked'))});$('.toggle_action').live('click',function(){$.toggleAction(this.href,false);return false;});$('.ajax_action').live('click',function(){var $this=$(this);$this.addClass('loading');$.mayContinue($this,function(){$.getJSON($this.attr('href')+'&authenticity_token='+$.get_auth_token(),function(response){$this.removeClass('loading');if(response.success){if($this.attr('rel')=='helptext'){$.toggleHelptext($this);}else if($this.hasClass('delete_link')){$this.parent().parent().slideUpRemove();}else if($this.hasClass('rm_field')){$this.parent().fadeOutRemove(600);}}else{$.ajax_error(response);$this.removeClass('loading');}});},function(){$this.removeClass('loading');});return false;});$('.post_link').live('click',function(){var $this=$(this);$.mayContinue($this,function(){$this.addClass('loading');$.post($this.attr('href')+'&authenticity_token='+$.get_auth_token(),function(response){$.with_json(response,function(data){$this.parent().fadeOutRemove(600);});$this.removeClass('loading');},'json');});return false;});$('.add_link','.partial_addable').live('click',function(){var $this=$(this),context='#'+$this.attr('rel'),partial_form=$($('.partial_form_html',context).html());$('input, select, checkbox',partial_form).each(function(){$(this).attr('disabled',false);});partial_form.hide().prependTo('.partial_forms_wrap',context).slideDown(600);$.bindPlugins();return false;});$('.cancel_link','.partial_addable').live('click',function(){$(this).parent().parent().slideUpRemove();return false;});$('.insert_partial').live('click',function(){var $this=$(this);$.get(this.href,function(response){$('#'+$this.attr('rel')).html(response);})
return false;});$('.inline_delete_link').live('click',function(){var $this=$(this);$.mayContinue($this,function(){if($this.attr('rel').split('_')[1]=='0')$this.parent().parent().slideUpAndRemove();else $('#'+$this.attr('rel')).slideUpAndRemove();});return false;});$('.select_on_focus').live('focus',function(){$(this).select();})
if($.on_page([['edit, new','views, forms, links']])){var scope_down='';if($.on_page([['edit, new','views']]))scope_down='owner';else scope_down='target';var scoping_fields=$('#scope_'+scope_down+'_fields','#body');if($('.scope_down_hidden',scoping_fields).val()){var scoper_id=$('.scope_down_hidden',scoping_fields).val();scoping_fields.children().each(function(){if(this.value==scoper_id)this.selected='selected';});}else scoping_fields.hide();$('.scope_dropdown','#scope_fields').live('change',function(){var $this=$(this);if($this.val()!=''){scoping_fields.show(100);scoping_dropdown=$('.scoping_dropdown',scoping_fields);scoping_dropdown.html('<option>Loading '+$this.val()+'...</option>');$.getJSON('/ajax/get_all',{model:$this.val(),authenticity_token:$.get_auth_token()},function(response){if(response.success){var args={attribute:'name',select_prompt:(scoping_dropdown.hasClass('no_prompt')?'':'Active Context')}
var option_tags=$.option_tags_from_model($this.val(),response.data,args);scoping_dropdown.html(option_tags);}else{scoping_dropdown.html('<option>Error Loading Records</option>')}});}else scoping_fields.hide(300);});$('#form_controller','#FormsForm').live('change',function(){fillInFormFieldSelectLists($(this).val());});$('.field_attr_name','#form_builder').live('filled',function(){$('.field_attr_name','#form_builder').each(function(){var $this=$(this),name=$this.prev('span.field_name').text();$this.children('option').each(function(){var $this_option=$(this);if($this_option.val()==name)$this_option.attr('selected',true);});});$(this).append('<option value="return_to">return_to</option>');});$('.delete_link','#form_builder').live('click',function(){var $this=$(this),field_id=$(this).attr('rel').replace('field_','');$this.parent().parent().html();return false;});}
if($.on_page([['edit','forms']])){var resource=$('#form_controller','#FormsForm').val();fillInFormFieldSelectLists(resource);}
$('#blast_off').live('click',function(){var $this=$(this),blast_type=$('input[name=blast_type]:checked','#blaster').val(),test_emails=$('#test_emails','#blaster').val(),ajax_loader=$.new_ajax_loader('after',$this).show();if(!$this.data('blasting')){$this.data('blasting',true);$.getJSON($this.attr('data-blast-path'),{blast_type:blast_type,test_emails:test_emails,authenticity_token:$.get_auth_token()},function(response){$.with_json(response,function(data){$.greyAlert(data,false);});$this.data('blasting',false);ajax_loader.hide();});}
return false;});$('.auto_pop_up_link').live('click',function(){var $this=$(this),div_id=$this.attr('data-div-id'),div=$('#'+div_id).clone(),ops=default_pop_up_options({title:$this.attr('title'),width:$this.attr('data-width'),height:$this.attr('data-height')});if(this.href.split('#')[1].length==0){var pop_up=$('<div id="pop_up" class="auto_pop_up '+div_id+'"></div>').append(div).dialog(ops);$('.tabular_content',pop_up).tabular_content().show();}else{get_pop_up_and_do(ops,{sub_partial:this.href},function(pop_up){$('.tabular_content',pop_up).tabular_content();});}
return false;});});$.option_tags_from_model=function(model_class,models,options){var attribute=options.attribute||'name',select_prompt=options.select_prompt||false,option_tags=select_prompt?'<option value="">'+select_prompt+'</option>':'';$.each(models,function(i){if(attribute=='name'&&!this[model_class]['name'])
attribute='title';else if(attribute=='title'&&!this[model_class]['title'])
attribute='name';option_tags+='<option value="'+this[model_class]['id']+'">('+this[model_class]['id']+') '+this[model_class][attribute]+'</option>';});return option_tags;}
$.option_tags_from_array=function(options,selected){var options_tags='';$.each(options,function(){options_tags+='<option'+(selected&&selected==this?'selected="selected"':'')+' value="'+this+'">'+this+'</option>';});return options_tags;}
$.with_json=function(response,success,error){if(response.success)(success||function(){}).call(this,response.data);else if(error&&error.call)error.call(this,response.data)
else $.ajax_error(response);}
$.log=function(msg){typeof(console!='undefined')?console.log(msg):alert(msg);}
$.ajax_error=function(response){if(typeof console=='undefined')$.greyAlert((typeof(response.data)=='undefined'?response:response.data));else console.log(response);}
$.on_page=function(route_sets){var i=route_sets.length,actions,controllers,route=[$('body').attr('class').split(' ')[0].replace('_action',''),$('body').attr('id').replace('_controller','')];while(i--){actions=route_sets[i][0].split(/,\W?/);controllers=route_sets[i][1].split(/,\W?/);var j=actions.length;while(j--){if(route[0]!=actions[j])continue;var k=controllers.length;while(k--){if(route[1]==controllers[k])return true;}}}}
$.enableEditor=function(){$('textarea.wysiwyg').each(function(i){var textarea=jQuery(this),toggle=jQuery('<a href="#" class="toggle right" id="toggle_'+i+'">Toggle Editor</a>');textarea.parent().parent().prepend(toggle);toggle.click(function(){CKEDITOR.replace(textarea.attr('name'));return false;});});}
$.get_auth_token=function(){var token=$('input[name=authenticity_token]').val();return token;}
$.validateAnimationAction=function(action){return/^show$|^hide$|^slideDown$|^slideUp$/.test(action);}
$.switch_action_hash=function(this_el,action,elementClass,contextClass){action=$.switch_actions(action);$(this_el).attr('href',$(this_el).attr('href').split('#')[0]+'#'+action+'_'+elementClass+'_'+contextClass);$(this_el).toggleClass('toggle_down');}
$.switch_actions=function(action){var action_sets=[['show','hide'],['fadeIn','fadeOut'],['slideDown','slideUp'],['addClass','removeClass']];var i=action_sets.length;while(i--){if(action_sets[i].indexOf(action)>=0)return action_sets[i][(action_sets[i].indexOf(action)^1)];}}
$.disable=function(disabler,disablee){$(disablee,disabler.parent().parent()).each(function(i){if(this.name!=disabler.attr('name'))$(this).attr('disabled',!disabler.data('enabled'));});}
$.disableToggle=function(disabler,disablees){disabler.data('enabled',!disabler.data('enabled'));$.disable(disabler,disablees);}
$.toggleAction=function(href,scroll_to_it){var url_hash=href.split('#');if(url_hash[1]){var url_hash=url_hash[1],action=url_hash.split('_')[0],element=url_hash.split('_')[1],context=url_hash.split('_')[2],target=$(element,context);if($.validateAnimationAction(action)){if(element.match(/^ov-.*/)){setTimeout(function(){$('a[rel='+element+']','#admin-box').click();},1);}else{var oContext=$(context);var actionLink=$('.toggle_action',oContext);if(actionLink.length)$.switch_action_hash($('.toggle_action',oContext),action,element,context);try{if(scroll_to_it)$(document).scrollTo(oContext,800);}catch(e){}
target[action]();}}}}
$.updateModels=function(e,ui,callback){var list_items=ui.item.parent().children(),$this=ui.item,query='';$(list_items).each(function(i){var model_class=this.id.split('_')[0],model_id=this.id.split('_')[1],model_attr=$this.attr('data-attr');query+='models['+i+'][model]='+model_class+'&models['+i+'][id]='+model_id+'&models['+i+'][attribute]='+model_attr+'&models['+i+'][value]='+i+'&';});$.post('/ajax/update_many',query,function(response){$.with_json(response,function(data){if(typeof callback=='function')callback.call(this,data);else $this.effect('bounce',{},200);});},'json');}
$.updateModel=function(path,params,callback){$.post(path,params,function(response){$.with_json(response,function(data){if(typeof callback=='function')callback.call(this,data);else alert(data);});},'json');}
$.updateUserStat=function(){if(typeof usssl_stat_path!='undefined')
$.post(usssl_stat_path,{browser_vars:$.browserVars(),_method:'put'});}
$.getModelAttributes=function(resource,callback){var attributes=[];$.getJSON('/ajax/get_attributes?model='+singularize(resource),function(response){$.with_json(response,function(data){if(callback&&typeof callback=='function')callback.call(this,data);else return data;});});}
$.injectOptionsInSelectLists=function(field_name_selects,option_tag_html){$.each(field_name_selects,function(){$(this).html(option_tag_html)});}
$.revertSettingsTextFieldToLabel=function(text_field,old_val){text_field.blur(function(){var this_field=$(this),new_val=this_field.val(),field_container=this_field.parent().parent(),setting_field=$('.setting_field input',field_container),old_field_name=setting_field.attr('name');this_field.fieldToLabel();setting_field.attr('name',old_field_name.replace(old_val.toLowerCase(),new_val.toLowerCase()))
if(field_container.hasClass('new_setting_field')){field_container.removeClass('new_setting_field').addClass('existing_setting_field');}});}
$.any=function(arr,callback){var has=false;$.each(arr,function(i){if(callback.call(this,i)){has=true;return;}});return has;}
$.disable_submit_btn=function(btn){return $.toggle_submit_btn(btn,true,.5);}
$.activate_submit_btn=function(btn){return $.toggle_submit_btn(btn,false,1);}
$.toggle_submit_btn=function(btn,toggle,fade){return $(btn).attr('disabled',toggle).fadeTo(300,fade);}
$.mayContinue=function(link,callback,else_callback){if(!link.hasClass('before_confirm')||(link.hasClass('before_confirm')&&$.greyConfirm(link.attr('title'),callback))){return true;}else if(typeof else_callback=='function'){else_callback.call(this,link);}}
$.open_map=function(map){map.show();var map_btn=$('#top_map_btn'),location=map_btn.attr('data-loc').split(','),lat=parseFloat(location[0]),lng=parseFloat(location[1]);$('span',map_btn).text('Hide Map');Gmap.checkResize();Gmap.setCenter(new GLatLng(lat,lng),12);}
$.get_param_value=function(key){var query=window.location.href.split('?')[1];if(query){var params=query.split('&'),value;$.each(params,function(){var key_val=this.split('=');if(key_val[0]==key){value=key_val[1];return;}});return value;}}
$.greyConfirm=function(msg,action,cancel,alert,error){var pop_up=$('<div id="pop_up" class="'+(typeof(alert)=='undefined'||!alert?'confirm_box':'alert_box')+'"></div>').dialog({title:alert?(error?'Alert':'Notice'):'Confirm',width:400,height:'auto',modal:true,resizable:false,close:function(){$(this).dialog('destroy')}});if(typeof(alert)!='undefined')
var btns='<a href="#" id="alert_ok" class="btn yes">'+(error?'Doh!':'Ok')+'</a>';else
var btns='<a href="#" id="confirm_yes" class="btn yes">Yes</a><a href="#" id="confirm_cancel" class="btn no">Cancel</a>';pop_up.html('<p>'+msg+'</p>'+btns);$('.btn',pop_up).click(function(){if(typeof(alert)=='undefined'||!alert){var confirm=$(this).text()=='Yes'?true:false;if(confirm&&typeof(action)=='function')action.call(this);else if(typeof cancel=='function')cancel.call(this);}
pop_up.dialog('destroy');return false;});$(document).keypress(enter_for_yes_escape_for_no);function enter_for_yes_escape_for_no(e){var key=e.which||e.keyCode;if(key==13){$('.yes',pop_up).click();$(document).unbind('keypress',enter_for_yes_escape_for_no);}}}
$.greyAlert=function(msg,error){if(typeof error=='undefined')error=true;$.greyConfirm(msg,null,null,true,error);}
$.authenticate_user_and_do=function(btn,callback,bypass){if(typeof bypass!='undefined'&&bypass){callback.call(this);}else{var pop_up=$('<div id="pop_up" class="confirm_box auth_box"></div>').dialog({title:'Authenticate Yourself',width:400,height:140,modal:true,resizable:false,close:function(){$(this).dialog('destroy').remove()}});pop_up.append($('#auth_yourself','#content').clone().css('display','block'));var input=$('input',pop_up).removeClass('invalid').focus();$('#confirm_yes',pop_up).click(function(){$(this).parents('form').submit();return false;});$('#auth_yourself',pop_up).submit(function(){var form=$('form','#pop_up').runValidation(),ajax_loader=$.new_ajax_loader('before',this),text=$('p',form).show();if(form.data('valid')&&!form.data('sending')){form.data('sending',true);ajax_loader.show();$.post(form.attr('action'),form.serialize(),function(response){$.with_json(response,function(data){pop_up.dialog('destroy').remove();callback.call(this,data);},function(data){$('.flash',form).remove();form.prepend('<div class="flash error">'+data+'</div>').find('.flash').css('position','static');form.data('sending',false);ajax_loader.hide();text.hide();input.val('').addClass('invalid').focus();});form.data('sending',false);ajax_loader.hide();},'json');}
return false;});}}
$.ajax_loaders={};$.new_ajax_loader=function(where,el,img){var el=$(el);if(el.data('ajax_id')&&(loader=$.ajax_loaders[el.data('ajax_id')])){return loader;}else{el.data('ajax_id',(new Date()).getTime());var loader=$($.ajax_loader_tag(img,el));$.ajax_loaders[el.data('ajax_id')]=loader;try{el[where](loader);return loader;}catch(e){$.log('new ajax loader failed: '+e);}}}
$.ajax_loader_tag=function(img,context){if(typeof img=='undefined')var img='ajax-loader-facebook.gif';var id=typeof(context)=='undefined'?'':'al_'+context.attr('id');return'<img src="http://s3.amazonaws.com/storagelocator/images/ui/'+img+'" alt="Loading..." class="ajax_loader" id="'+id+'" />';}
$.setInterval=function(callback,interval){setTimeout(function(){callback.call(this);setTimeout(arguments.callee,interval);},interval)}
$.setup_autocomplete=function(els,context){if(typeof els=='undefined')var $autocompleters=$('.autocomplete');else if(els&&typeof(context)=='undefined')var $autocompleters=$(els);else var $autocompleters=$(els,context);var $autcompleted={};if($autocompleters.length>0){$autocompleters.each(function(){var $this=$(this),rel=$this.attr('rel'),info=rel.split('|')[0],minLen=rel.split('|')[1],model=info.split('_')[0],method=info.split('_')[1];if(!$autcompleted[rel]){$.getJSON('/ajax/get_autocomplete',{'model':model,'method':method},function(response){if(response.success&&response.data.length>0){$autcompleted[rel]=response.data;$this.autocomplete({source:response.data,minLength:minLen});}else $.ajax_error(response);});}});}}
var stuff_sort_inverse=false;$.sort_stuff=function(sort_link,elements,selector,sortFunc){sort_link.addClass(stuff_sort_inverse?'down':'up');sort_link.removeClass(stuff_sort_inverse?'up':'down');elements.sortElements(function(a,b){return sortFunc(a,b);},function(){return $(this).children(selector)[0];});stuff_sort_inverse=!stuff_sort_inverse;}
$.fn.disabler=function(d){var disablees=d||'input, textarea, select, checkbox, radio';return this.each(function(){var $this=$(this);$this.data('enabled',$this.is(':checked'));$.disable($this,disablees);$this.change(function(){$.disableToggle($this,disablees)});});}
$.fn.anchorDispatch=function(){var url_hash,listeningElement,anchor;url_hash=window.location.href.split('#')[1];if(!url_hash)
return false;if($.validateAnimationAction(url_hash.split('_')[0]))
return false
listeningElement=$('#'+url_hash.split('_')[0]);if(!listeningElement)
return false;if(url_hash.split('-')[1])anchor=$('#'+url_hash.split('-')[1]);return this.each(function(i){if(this.id==listeningElement.attr('id')){listeningElement.show();$('.collapseable',listeningElement).show().removeClass('hide');$('.toggle_action',listeningElement).addClass('toggle_down');}
if(anchor){$(document).scrollTo(anchor,2000);$('#'+url_hash.split('-')[0]).effect('highlight',{color:'#87c881'},7000);}});}
$.fn.rowCheckable=function(){return this.each(function(){var $this=$(this),checkbox=$('input[type=checkbox]',$this).eq(0);$this.live('click',function(e){if(e.target.tagName==e.currentTarget.tagName){checkbox.trigger('change').attr('checked',!checkbox.is(':checked'));}});});}
$.fn.paneSwitcher=function(){return this.each(function(){var context=$(this).parent().parent().parent();this.checked?$('#pane_0',context).hide():$('#pane_1',context).hide();$(this).change(function(){$('.pane_switchable',context).slideToggle();});});}
$.fn.toggleDiv=function(){return this.each(function(){var $this=$(this);var sibling=$this.parent().next('.toggle_this');this.checked?sibling.show():sibling.hide();$this.change(function(){sibling.toggle();sibling.find('input, textarea, select').focus();});});}
$.fn.textFieldable=function(text_field_html,callback){return this.each(function(){var $this=$(this),$text_field=$(text_field_html);$this.parent().html($text_field);$text_field.focus();$.bindPlugins();if(typeof callback=='function')callback.call(this,$text_field);});}
$.fn.fieldToLabel=function(){return this.each(function(){var $this=$(this),label='<label for="'+$this.attr('name')+'" class="block w110 small_round textFieldable">'+$this.val()+'</label>';$this.parent().prepend(label);$this.remove();});}
$.fn.slideUpAndRemove=function(speed){if(typeof speed=='null')speed=300;return this.each(function(){var $this=$(this);$this.slideUp(speed,function(){$this.remove();});});}
$.fn.animOpacity=function(){return this.each(function(){var $this=$(this),orig_opacity=$this.css('opacity');$this.hover(function(){$this.stop().animate({'opacity':1},300);},function(){$this.stop().animate({'opacity':orig_opacity},150);});});}
$.fn.linkDiv=function(){return this.each(function(){var $this=$(this),href=$this.find('a').attr('href');$this.click(function(){if(href)window.location=href;});});}
$.fn.fillWithParam=function(){var params=window.location.href.split('?')[1];if(!params)return false;return this.each(function(){var $this=$(this),value=false,attr=$this.attr('param')||$this.attr('rel');$.each(params.split('&'),function(){if(this.split('=')[0]==attr){value=this.split('=')[1];return;}});if(value)
$this.attr('value',decodeURIComponent(value.replace(/\+/g,'%20'))).removeClass('hint_text').attr('disabled',false);if($this.hasClass('focus_me'))
$this.focus();});}
$.fn.appendParamAndGo=function(){return this.each(function(){var $this=$(this);$this.click(function(){var key=$this.attr('rel').split('-')[0]
val=$this.attr('rel').split('-')[1],href=window.location.href,new_href='',has_param=href.indexOf('?')>=0,param=(has_param?'&':'?')+key+'='+val;if(!val)return false;if(href.indexOf(key)>=0){new_href=href.split(key)[0].replace(/\&$/,'')+param;}else{new_href=href+param}
if(href.indexOf(val)<0)
window.location=new_href;});});}
$.fn.openDiv=function(){return this.each(function(){var $this=$(this),div_to_open=$this.attr('rel');$this.click(function(){$('#'+div_to_open).slideToggle(600);$this.parent('.bg').toggleClass('expanded');if($this.hasClass('toggle_right'))$this.toggleClass('toggle_down');return false;});});}
$.fn.accordion=function(){return this.each(function(){$(this).click(function(){var $this=$(this),info_div=$('.info',$this.parent().parent());$('a',$this.parent().parent().parent()).removeClass('active');$('.info').slideUp();if(info_div.is(':hidden')){$this.addClass('active');info_div.slideDown().children().hide().fadeIn('slow');}else{$this.removeClass('active');info_div.slideUp();}
return false;})});}
$.fn.tabular_content=function(){function what_action(el){if(el.hasClass('slide'))return['slideUp','slideDown','slow'];else if(el.hasClass('fade'))return['fadeOut','fadeIn','slow'];else return['hide','show',null];}
return this.each(function(){var $this=$(this),tabs=$('.tabular',$this).length>0?$('.tabular',$this):$('.tabular'),panels=$('.tab_content',$this),action=what_action($this);tabs.find('li').eq(0).addClass('active');panels.eq(0).show();$('a',tabs).click(function(){panels[action[0]](action[2]).removeClass('active');$('li, a',tabs).removeClass('active');$(this).addClass('active').parent().addClass('active');$('#'+$(this).attr('rel'),$this)[action[1]]((action[0]=='hide'?null:'slow')).addClass('active');return false;});});}
$.liveSubmit=function(){$.each(arguments,function(){$(eval("'"+this+"'")).live('click',function(){$(this).parents('form').submit();return false;});});}
$.fn.clickOnLoad=function(){return this.each(function(){$(this).click();});}
$.fn.formatPhoneNum=function(){if($.browser.msie)return;return this.each(function(){$(this).keyup(function(e){var input=$(this),allowed_keys=[9,8,46];if(e.which==189||e.which==109){input.val(input.val().substring(0,input.val().length-1));}
if(allowed_keys.indexOf(e.which)<0&&isNaN(input.val().replace('-','').replace('-',''))){input.val(input.val().substring(0,input.val().length-1));}else if(allowed_keys.indexOf(e.which)<0&&input.val().length>=3&&input.val().length<7&&input.val().indexOf('-')<0){input.val(input.val().substring(0,3)+'-'+input.val().substring(3));}else if(allowed_keys.indexOf(e.which)<0&&input.val().length>=7&&input.val().indexOf('-')<7){input.val(input.val().substring(0,7)+'-'+input.val().substring(8));}});});}
$.fn.save_state=function(){return this.each(function(){var form=$(this);form.data('state',form.serialize());});}
$.fn.state_changed=function(){var prev_state=this.data('state');if(!prev_state)return false;return prev_state!=this.serialize();}
$.fn.txt_loader=function(options){function increment_txt(el,txt){var t=el.text();el.text(t+txt);}
return this.each(function(){var $this=$(this).text(''),count=0,settings={txt:'.',interval:1000,limit:3};if(typeof options=='undefined')
var options={}
$.extend(settings,options);setInterval(function(){if(count<settings.limit){increment_txt($this,settings.txt);}else{$this.text('');count=-1;}
count++;},settings.interval);});}
$.fn.shimmy=function(){return this.each(function(){});}
$.fn.slideUpRemove=function(speed,callback){$.fn.animAndRemove.call(this,'slideUp',speed,callback);}
$.fn.fadeOutRemove=function(speed,callback){$.fn.animAndRemove.call(this,'fadeOut',speed,callback);}
$.fn.animAndRemove=function(anim,speed,callback){return this.each(function(){$(this)[anim](speed||'fast',function(){$(this).remove();if(typeof callback=='function')
callback.call(this);});});}
$.fn.autoNext=function(){return this.each(function(){var $this=$(this);if(typeof $this.attr('maxlength')=='undefined')return;var form=$this.parents('form'),inputs=$('input, textarea, select',form);$this.keyup(function(){var input=$(this);if(input.val().length==input.attr('maxlength')&&input.val()!=input.attr('title')){input.blur();inputs.eq(inputs.index(input)+1).focus();}});});}
$.fn.fadeOutLater=function(fade_speed,timeout,callback){return this.each(function(){var $this=$(this);setTimeout(function(){$this.fadeOut(fade_speed,callback);},timeout||1000);});}
$.toggleHelptext=function(clickedLink){$('.inner','.'+clickedLink.attr('rel')).stop().slideToggle().toggleClass('hide');$('.ajax_action',clickedLink.parent()).toggleClass('hide');clickedLink.parent('.helptext_options').toggleClass('bg_green');}
$.bindPlugins=function(){$('.hintable').hinty();$('form').formBouncer();}
function fillInFormFieldSelectLists(resource){var $field_name_selects=$('.field_attr_name','#form_builder');if($field_name_selects.length){$.injectOptionsInSelectLists($field_name_selects,'<option>Loading..</option>');$.getModelAttributes(resource,function(attributes){$.injectOptionsInSelectLists($field_name_selects,$.option_tags_from_array(attributes));$field_name_selects.trigger('filled');});}}
function capitalize(string){if(typeof string!='undefined'){return string.substring(0,1).toUpperCase()+string.substring(1);}}
function titleize(string){if(typeof string!='undefined'){var parts=string.split(' '),titleized='';for(var i=0,len=parts.length;i<len;i++){titleized+=capitalize(parts[i])
if(i<len)titleized+=' ';}
return titleized;}}
function default_pop_up_options(options){return{title:options.title,width:options.width||785,height:options.height,resizable:(typeof options.resizable=='undefined'?false:options.resizable),modal:(typeof options.modal=='undefined'?true:options.modal),close:options.close||function(){$('.ajax_loader').hide();$(this).dialog('destroy').remove();}};}
function get_pop_up_and_do(options,params,callback){var params=params||{}
params.partial=params.partial||'/shared/pop_up';$.get('/ajax/get_multipartial',params,function(response){var pop_up=$(response).dialog({title:options.title,width:options.width||785,height:options.height,resizable:false,modal:options.modal,close:function(){$('.ajax_loader').hide();$(this).dialog('destroy').remove();}});if(typeof callback=='function')
callback.call(this,pop_up);});}
function get_partial_and_do(params,callback){var params=params||{}
params.partial=params.partial||'/shared/pop_up_box';$.get('/ajax/get_partial',params,function(response){if(typeof callback=='function')callback.call(this,response);});}
var GreyShow=function(options){var self=this;this.context=options.context;this.slides=options.slides;this.delay=options.delay;this.num_slides=options.slides.length;this.time_int=0;this.start=function(){self.current=0;self.startSlide();}
this.startSlide=function(){if(typeof self.slides[self.current].start=='function')
self.slides[self.current].start.call(this,self);self.hidePrevSlide();self.slide_objects=self.slides[self.current].objects;self.current_object=0;self.runObject(self.slide_objects[0]);}
this.hidePrevSlide=function(){var prev=self.current==0?self.num_slides-1:self.current-1;for(var i=0,len=self.slides[prev].objects.length;i<len;i++){var $object=$('#'+self.slides[prev].objects[i].id);$object.fadeOut(900);}}
this.runObject=function(o){var $object=$('#'+o.id);$object.children().hide();if(typeof o.callback=='function')
o.callback.call(this,$object,self);$object[o.action](o.speed,function(){self.nextObject(o);});}
this.nextObject=function(o){self.current_object++;if(self.slide_objects[self.current_object]){setTimeout(function(){self.runObject(self.slide_objects[self.current_object]);},o.delay);}else if(typeof self.slides[self.current].end=='function'){setTimeout(function(){self.slides[self.current].end.call(this,self);},self.delay);}}
this.gotoSlide=function(n){self.current=n;if(n==self.num_slides){self.current=0;self.gotoSlide(0);}else self.startSlide();}}
var GreyWizard=function(container,settings){var self=this;self.form_data={};self.settings=settings;self.slide_data=settings.slides;self.slides_class=settings.slides_class||'workflow_step',self.nav_id=settings.nav_id||'workflow_nav',self.num_slides=self.slide_data.length;self.workflow=$(container);self.title_bar=$('#ui-dialog-title-pop_up',self.workflow.parent().parent());self.width=self.workflow.width();self.height=self.workflow.height();self.slides=$('.'+self.slides_class,self.workflow).each(function(){$(this).data('valid',true);});self.spacer=settings.spacer||100;self.pad_left=settings.pad_left||15;self.slide_speed=settings.slide_speed||1500,self.btn_speed=settings.btn_speed||900,self.fade_speed=settings.fade_speed||1000,this.begin_workflow_on=function(step){self.workflow.parents('#pop_up').show();self.nav_bar=$('#'+(self.nav_id||'workflow_nav'),self.workflow).children().hide().end();self.current=step||0;self.current_slide=$('#'+self.slide_data[self.current].div_id,self.workflow);self.skipped_first=step>0?true:false;self.set_slides();self.set_nav();self.nav_bar.find('.next, .skip').click(self.next);self.nav_bar.find('.back').click(self.prev);if(self.title_bar.length)self.title_bar.change(function(){if(self.slide_data[self.current].pop_up_title)$(this).text(self.slide_data[self.current].pop_up_title);else $(this).text(self.settings.title);}).trigger('change');if(typeof self.slide_data[self.current].action=='function')self.slide_data[self.current].action.call(this,self);}
this.set_slides=function(){if(typeof set_display=='undefined')set_display=false;self.slides.css({'margin-right':self.spacer+'px'});self.workflow.children('.items').width(self.num_slides*(self.width+self.spacer+self.pad_left)+3);self.workflow.scrollable({speed:1000,circular:false,next:'.none',prev:'.none'}).data('scrollable').seekTo(self.current,1);if(self.settings.set_slides){self.slide_steps=$('<div id="slide_nav" />').appendTo(self.workflow.parent())
var step_display='',active_slides=self.num_slides-(self.skipped_first?1:0),slide_tab_width=parseInt(100/active_slides)-(self.skipped_first?3:2.68),done_skipping=false;for(var i=0;i<self.num_slides;i++){if(self.skipped_first&&!done_skipping){done_skipping=true;continue;}
step_display+='<div id="tab_step_'+i+'" class="slide_display '+(self.current==i?' active':'')+(i==(self.skipped_first?1:0)?' first':(i==self.num_slides-1?' last':''))+'" style="width:'+slide_tab_width+'%;">'+'<p>Step '+(i+1)+'</p>'+
(typeof self.slide_data[i].slide_display!='undefined'?self.slide_data[i].slide_display:'')+'</div>';}
self.slide_steps.html(step_display);}}
this.set_nav=function(){if(typeof self.slide_data[self.current]!='undefined'){$.each(self.slide_data[self.current].nav_vis,function(){var btn=$('.'+this[0],self.nav_bav),action=this[1];if(action){if(typeof action=='function')action.call(this,btn,self);else if(typeof action=='string')btn[action]((action=='hide'||action=='show'?null:self.btn_speed));}});}
if(self.settings.set_slides)setTimeout(function(){$('.slide_display',self.workflow.parent()).removeClass('active');$('#tab_step_'+self.current,self.workflow.parent()).addClass('active');},self.fade_speed);}
this.may_move=function(step){var validated=true;if(typeof self.slide_data[self.current].validate=='function'&&step>0)
validated=self.slide_data[self.current].validate.call(this,self);return validated&&((self.current+step)>=0&&(self.current+step)<self.num_slides)&&(step<0||(step>0&&!$('.next',self.workflow).data('done')));}
this.next=function(step){self.move(typeof(step)=='number'?step:1);return false;}
this.prev=function(step){self.move(typeof(step)=='number'?step:-1);return false;}
this.move=function(step){if(self.may_move(step)){if(step>0)$('#tab_step_'+self.current,self.workflow.parent()).addClass('done');self.current+=step;self.workflow.data('scrollable').seekTo(self.current);self.set_nav();self.title_bar.trigger('change');if(typeof self.slide_data[self.current].action=='function')
self.slide_data[self.current].action.call(this,self);}else if(self.current==self.num_slides-1){if(typeof(self.settings.finish_action)=='function')
self.settings.finish_action.call(this,self);else if(self.settings.finish_action=='close')
self.workflow.parent().dialog('close').remove();}}}
var Ajax=function(){};Ajax.Request=function(url,params){$.post(url,params.parameters);}
String.prototype.replaceAll=function(find,replace){var temp=this,index=temp.indexOf(find);while(index!=-1){temp=temp.replace(find,replace);index=temp.indexOf(find);}
return new String(temp);}
function capitalize_addr(addr){if(typeof addr!='string')return;var capped='',parts=addr.split(' '),dirs=['ne','nw','se','sw'];$.each(parts,function(i){if($.inArray(this.toLowerCase(),dirs)>=0)
capped+=this.toUpperCase();else
capped+=capitalize(this);if(i<parts.length-1)capped+=' ';});return capped;}
var dateFormat=function(){var token=/d{1,4}|m{1,4}|yy(?:yy)?|([HhMsTt])\1?|[LloSZ]|"[^"]*"|'[^']*'/g,timezone=/\b(?:[PMCEA][SDP]T|(?:Pacific|Mountain|Central|Eastern|Atlantic) (?:Standard|Daylight|Prevailing) Time|(?:GMT|UTC)(?:[-+]\d{4})?)\b/g,timezoneClip=/[^-+\dA-Z]/g,pad=function(val,len){val=String(val);len=len||2;while(val.length<len)val="0"+val;return val;};return function(date,mask,utc){var dF=dateFormat;if(arguments.length==1&&Object.prototype.toString.call(date)=="[object String]"&&!/\d/.test(date)){mask=date;date=undefined;}
date=date?new Date(date):new Date;if(isNaN(date))throw SyntaxError("invalid date");mask=String(dF.masks[mask]||mask||dF.masks["default"]);if(mask.slice(0,4)=="UTC:"){mask=mask.slice(4);utc=true;}
var _=utc?"getUTC":"get",d=date[_+"Date"](),D=date[_+"Day"](),m=date[_+"Month"](),y=date[_+"FullYear"](),H=date[_+"Hours"](),M=date[_+"Minutes"](),s=date[_+"Seconds"](),L=date[_+"Milliseconds"](),o=utc?0:date.getTimezoneOffset(),flags={d:d,dd:pad(d),ddd:dF.i18n.dayNames[D],dddd:dF.i18n.dayNames[D+7],m:m+1,mm:pad(m+1),mmm:dF.i18n.monthNames[m],mmmm:dF.i18n.monthNames[m+12],yy:String(y).slice(2),yyyy:y,h:H%12||12,hh:pad(H%12||12),H:H,HH:pad(H),M:M,MM:pad(M),s:s,ss:pad(s),l:pad(L,3),L:pad(L>99?Math.round(L/10):L),t:H<12?"a":"p",tt:H<12?"am":"pm",T:H<12?"A":"P",TT:H<12?"AM":"PM",Z:utc?"UTC":(String(date).match(timezone)||[""]).pop().replace(timezoneClip,""),o:(o>0?"-":"+")+pad(Math.floor(Math.abs(o)/60)*100+Math.abs(o)%60,4),S:["th","st","nd","rd"][d%10>3?0:(d%100-d%10!=10)*d%10]};return mask.replace(token,function($0){return $0 in flags?flags[$0]:$0.slice(1,$0.length-1);});};}();dateFormat.masks={"default":"ddd mmm dd yyyy HH:MM:ss",shortDate:"m/d/yy",mediumDate:"mmm d, yyyy",longDate:"mmmm d, yyyy",fullDate:"dddd, mmmm d, yyyy",shortTime:"h:MM TT",mediumTime:"h:MM:ss TT",longTime:"h:MM:ss TT Z",isoDate:"yyyy-mm-dd",isoTime:"HH:MM:ss",isoDateTime:"yyyy-mm-dd'T'HH:MM:ss",isoUtcDateTime:"UTC:yyyy-mm-dd'T'HH:MM:ss'Z'"};dateFormat.i18n={dayNames:["Sun","Mon","Tue","Wed","Thu","Fri","Sat","Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],monthNames:["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec","January","February","March","April","May","June","July","August","September","October","November","December"]};Date.prototype.format=function(mask,utc){return dateFormat(this,mask,utc);};$(function(){$('a','#sl-tabs-nav').click(function(){window.location.hash=this.href.split('#')[1];});if($.on_page([['profile, show','listings']])){if(window.location.hash!=''){setTimeout(function(){$('a[href="'+window.location.hash+'"]','#sl-tabs-nav').click();},1);}}
var listings=$('.listing','#client_listing_box');if(listings.length>0){listings.each(function(){$('.progressbar',this).progressbar({value:parseInt($('.percent',this).text())});});}
$('a','#address_sort').live('click',function(){var $this=$(this),listings=$('.listing','#client_listing_box');$.sort_stuff($this,listings,'.inner',function(a,b){var a1=parseInt($('.rslt-contact p',a).text()),b1=parseInt($('.rslt-contact p',b).text());return a1>b1?(stuff_sort_inverse?1:-1):(stuff_sort_inverse?-1:1);});return false;});$('form.size_form','#unit_sizes').live('submit',function(){var form=$(this).runValidation(),ajax_loader=$('.ajax_loader',form);if(form.data('valid')&&!form.data('saving')){form.data('saving',true);ajax_loader.show();$('.cancel_link',form).hide();$.post(form.attr('action'),form.serialize(),function(response){$.with_json(response,function(data){var new_size=$(data);form.after(new_size);$('.sl-table',new_size).effect('highlight',3000);if(form.attr('data-edit')=='1')form.remove();});form.data('valid',null);form.data('saving',false);ajax_loader.hide();});}
return false;});$('.cancel_link','.size_form').live('click',function(){$(this).parents('.size_form').fadeOutRemove();return false;});$('.edit_size','.sl-table-wrap').live('click',function(){var $this=$(this),size=$this.parents('.sl-table-wrap'),ajax_loader=$('.ajax_loader',$this.parent().next()).show();delete_link=$('.delete_link',size).hide();get_partial_and_do({partial:'sizes/form',model:'Listing',id:$('#listing_id').val(),sub_model:'Size',sub_id:$this.attr('data-size-id')},function(response){$.with_json(response,function(partial){var form=$(partial),price=$('#size_price',form);price.val((parseFloat(price.val())/100.0).toFixed(2)).focus();size.after(form);size.hide();$('#size_price',form).val(parseInt($('#size_price',form).val()))
$('.cancel_link',form).click(function(){ajax_loader.hide();delete_link.show();form.hide();size.show();return false;});});});return false;});$('.delete_link','.sl-table-wrap').live('click',function(){var $this=$(this),ajax_loader=$('.ajax_loader',$this.parent());$.greyConfirm('Are you sure you want to delete this unit size?',function(){$this.hide();ajax_loader.show();$.post($this.attr('href'),{_method:'delete'},function(response){$.with_json(response,function(data){$this.parents('.sl-table-wrap').fadeOutRemove();});});});return false;});$('.select_predefined_size','#sl-tabs-predefined-sizes-in').live('click',function(){$(this).parents('.sl-table').click();return false;});$('.sl-table','#sl-tabs-predefined-sizes-in').click(function(){var predef=$(this).parent(),predef_id=predef.attr('id').replace('PredefinedSize_',''),ajax_loader=$('.ajax_loader',predef);if(!predef.data('adding')){predef.data('adding',true);ajax_loader.show();$.post('/listings/'+$('#listing_id').val()+'/add_predefined_size',{'predef_id':predef_id},function(response){$.with_json(response,function(data){var size=$(data);$('.uncollapse','#sl-tabs-sizes-in').append(size);$('.sl-table',size).addClass('active').effect('highlight',1500).find('#size_price').focus();});predef.data('adding',false);ajax_loader.hide();});}});$('#request_review_form').live('submit',function(){var form=$(this).runValidation(),ajax_loader=$('.ajax_loader',form);$('.success_msg',form).remove();if(form.data('valid')&&!form.data('sending')){form.data('saving',true);ajax_loader.show();$.post(form.attr('action'),form.serialize(),function(response){$.with_json(response,function(data){form[0].reset();$('textarea',form).each(function(){$(this).focus().blur();});ajax_loader.before("<p class='success_msg'>Your message was sent!</p>");});form.data('sending',false);ajax_loader.hide();});}
return false;});$('.attr_edit','.authenticated').live('click',function(){var $this=$(this).css('display','inline'),container=$this.parent(),rel=$this.attr('rel'),cancel_btn=$('.cancel_btn',$this.parent());if($this.text()=='Edit'){cancel_btn.show();$this.text('Save').data('saving',false);$('.attr_wrap',container).children('.field_group').each(function(){var model=$(this).attr('rel');$('.value',this).each(function(){var field=$(this).hide(),attr=field.attr('rel');field.after('<input type="text" name="'+model+'['+attr+']" class="small_text_field i '+attr+'" value="'+field.text()+'" title="'+capitalize(attr.replaceAll('_',' '))+'" />');});});cancel_btn.click(function(){$this.text('Edit').data('saving',false).attr('style','');cancel_btn.hide();$('.value',container).show();$('input.i',container).remove();return false;});}else if($this.text()=='Save'&&!$this.data('saving')){$this.data('saving',true);$('.ajax_loader',container).show();var hidden_form=$('form[rel='+rel+']',container);$.clone_and_attach_inputs('input.i',container,hidden_form);$.post(hidden_form.attr('action'),hidden_form.serialize(),function(response){$.with_json(response,function(data){$('input.i',container).each(function(){var input=$(this),val=input.val();input.prev('.value').text(val).show();input.remove();});$this.text('Edit').attr('style','');cancel_btn.hide();});$this.data('saving',false);$('.ajax_loader',container).hide();});}
return false;});$('.amenity input','#tab3').button().click(function(){var $this=$(this),form=$this.parents('form.amenity'),ajax_loader=$.new_ajax_loader('after',this).show();$.post(form.attr('action'),function(response){$.with_json(response);ajax_loader.hide();},'json');});$('input[type=text]','#new_facility_feature').focus(function(){$(this).next('#facility_feature_submit').show('fast');});$('input[type=text]','#new_facility_feature').blur(function(){var $this=$(this);setTimeout(function(){$this.next('#facility_feature_submit').hide('fast')},300);});$('#new_facility_feature').submit(function(){var form=$(this);$.post(form.attr('action'),form.serialize(),function(response){$.with_json(response,function(data){$.log(data);});});return false;});$('.copy_all a').live('click',function(){var $this=$(this).fadeTo(600,.5);if(!$this.data('saving')){$this.data('saving',true);var ajax_loader=$.new_ajax_loader('after',$this).show().css({'float':'left','margin':'0 10px'});$.post($this.attr('href'),function(response){$.with_json(response,function(data){$this.parent().after('<span class="success_msg" style="float:left;margin:0 10px">Saved!</span>');setTimeout(function(){$('.success_msg',$this.parent()).fadeOutRemove(1000);},2000);});$this.data('saving',false)
$this.fadeTo(300,1);ajax_loader.hide();},'json');}
return false;});$('.slider').each(function(){var $this=$(this),value=$('.slider_val',$this.parent()).val();$this.slider({max:50,min:5,step:5,animate:true,value:value,start:function(e,ui){var slider=$('.slider_val',$(e.target).parent());if(slider.attr('disabled'))slider.attr('disabled',false);},slide:function(e,ui){$('.slider_val',$(this).parent()).val(ui.value);}});});$('li.enabled','li.rslt-price').live('click',function(){var $this=$(this),check=$('input.unit_size',$this);check.attr('checked',true);$('li.enabled',$this.parent()).removeClass('selected');$this.addClass('selected');});$compare_btns=$('input[name=compare]','.listing');$compare_btns.live('click',function(){var compare=$(this),listing=compare.parents('.listing'),blank_compare_href=$('.compare a','.listing').eq(0).attr('href'),id=compare.val(),marker,compare_links;if(typeof Gmaps_data!='undefined')marker=getMarkerById(id);if(compare.is(':checked')){listing.addClass('active');compare_links=$('.compare a','.listing.active');if(compare_links.length>=2){$('a','.active .compare').show();$('label','.active .compare').hide();}
if(marker){marker.GmapState='selected';highlightMarker(marker);}}else{$('a',compare.parent()).hide();$('label',compare.parent()).show();compare_links=$('.compare a','.listing.active');if(compare_links.length<=2){$('a','.active .compare').hide();$('label','.active .compare').show();}
listing.removeClass('active');if(marker){marker.GmapState='';unhighlightMarker(marker);}}});$('input[name=compare]','.compare').each(function(){var $this=$(this);if($this.is(':checked'))$this.attr('checked',false);});$('a','.compare').live('click',function(){var $this=$(this).hide(),orig_href=$this.attr('href'),compares=$('input:checked','.compare'),ajax_loader=$.new_ajax_loader('after',$this).show(),ids=[];compares.each(function(){var context=$(this).parents('.listing'),special_id=$('.special_txt',context).attr('data-special-id'),size_id=$('ul.dnp input.unit_size:checked',context).val();ids.push(this.value+'x'+size_id+'x'+special_id);});this.href+=ids.join('-');$.getJSON(this.href,function(response){$.with_json(response,function(data){var pop_up=$('<div id="pop_up">'+data['html']+'</div>').dialog(default_pop_up_options({title:'Comparing '+compares.length+' Facilities',width:'auto',height:'auto',modal:true})),ajax_loader=$('.ajax_loader',pop_up).show();$.calculatePrice(pop_up);$.setGmap(data.maps_data,'compare_map');});$this.show().attr('href',orig_href);ajax_loader.hide();});return false;});$('.special_txt').live('switched',function(){var spec_txt=$(this),ajax_loader=$.new_ajax_loader('html',$('#calcfor_'+spec_txt.attr('data-listing-id'))).show(),btns=$('.calc_params','#'+spec_txt.attr('data-context'));btns.each(function(){var special_id=spec_txt.attr('data-special-id');this.href=this.href.replace(/\&special=\d+/,'&special='+special_id);$(this).attr('data-special-id',special_id);});$.calculatePrice('#calc_params_for_'+spec_txt.attr('data-listing-id'));});$('.specializer','.specializer_wrap').live('click',function(){var $this=$(this),specials=$('.more_specials',$this.parent());if($this.text()=='more'){$this.text('less');specials.addClass('show_specials').show().css({'top':'-'+specials.height()+'px','right':'-'+(specials.outerWidth()/2)+'px'});}else{$this.text('more');specials.hide().css('right',0);}
return false;});$('.special_txt','.more_specials').live('click',function(){var $this=$(this),context=$this.parents('.specializer_wrap'),active_special=$('.special_txt.active',context),special_clone=active_special.clone().removeClass('active'),more=$this.parents('.more_specials').hide().css('right',0);;active_special.replaceWith($this.clone().addClass('active'));$this.replaceWith(special_clone);$('.specializer',more.parent()).text('more');$('.special_txt.active',context).trigger('switched');});$('.more_results').live('click',function(){var $this=$('.more_results'),this_form=$this.parents('form'),results_wrap=$('#rslt-list-bg'),plus_sign=$this.find('span > span').hide(),ajax_loader=$('.ajax_loader',$this).show(),last_index=parseInt($('.num_icon','.listing:last').text())+1,page=$('input[name=page]',$this.parent()).eq(0).val();if(!this_form.data('submitting')){this_form.data('submitting',true);$.getJSON(this_form.attr('action'),this_form.serialize(),function(response){$.with_json(response,function(data){for(var i=0,len=data.length;i<len;i++){var listing=$(data[i]);$('.num_icon',listing).text(last_index+i);results_wrap.append(listing);}
$('input[name=page]').val(parseInt(page)+1);var range=$('.results_range'),range_start=parseInt(range.eq(0).text().split('-')[0]),range_end=parseInt(range.eq(0).text().split('-')[1]),per_page=parseInt($('#per_page').text()),total=parseInt($('.results_total').eq(0).text()),remaining=total-(range_end+per_page);range_end+=parseInt($('#per_page').text());if(range_end>=total)range_end=total;range.text(range_start+'-'+range_end);if(remaining<=0)$this.hide();if(remaining<per_page)$this.find('span').html('<span class="plus">+</span> Show '+remaining+' more');});ajax_loader.hide();plus_sign.show();this_form.data('submitting',false);});}
return false;});select_first_size_option();$.activateSizeSelect('#narrow_results_form');$('.open_tab','.tabs').live('click',function(){var $this=$(this),$panel=$('.panel',$this.parent().parent().parent());$('.open_tab').text('+');if(!$this.data('active')){$('.tab_link[rel=map]',$this.parent().parent()).click();$this.data('active',true);$this.text('-');}else{$panel.slideUp();$('.tab_link, .listing, .panel, .tabs li').removeClass('active');$this.data('active',false);$('.open_tab').text('+');}
return false});$('.reserve_btn, .request_btn','.listing').live('click',function(){var $this=$(this),new_href=$this.attr('href').replace('/sizes',($this.hasClass('reserve_btn')?'/reserve':'/info_request')),context=$this.parents('.inner'),unit_size=$(':radio:checked',context),special=$('.special_txt.active',context);if(unit_size.length){var ar=(special.length==1?'[0]':'');$this.attr('href',new_href+'&sub_model'+ar+'=Size&sub_id'+ar+'='+unit_size.val());$this.attr('rel','reserve');}
if(special.length==1)
$this[0].href+='&sub_model[1]=PredefinedSpecial&sub_id[1]='+special.attr('data-special-id');});$('.tab_link','.listing').live('click',function(){$('.open_tab',this).data('active',false);var $this=$(this),$listing=$this.parents('.listing'),$panel=$('.panel',$listing).addClass('active'),$progress=$('.progress',$listing);if($.clicked_on_different_tab($this,$listing,$panel)){$progress.addClass('active').animate({'margin-top':0},'fast');$panel.attr('rel',this.rel);$.getJSON(this.href,function(response){$.with_json(response,function(data){$('.tab_link, .listing, .panel').removeClass('active');$('li','.tabs').removeClass('active');$this.parent().addClass('active');$this.addClass('active');$listing.addClass('active');$panel.addClass('active');$('.panel:not(.active)').slideUp();$panel.html(data);$('.listing:not(.active) .open_tab').text('+');$('.open_tab',$listing).data('active',true).text('-');if($panel.is(':hidden')){$panel.slideDown(900,function(){if($(window).height()<650)$(window).scrollTo($listing,{speed:1000});});}
$('.progress','.listing').removeClass('active').animate({'margin-top':'-16px'},'fast');if($this.attr('rel')=='map'){$('.hintable',$panel).hinty();var $map_wrap=$('.map_wrap',$panel),latlng=$map_wrap.attr('rel').split(',').map(function(i){return parseFloat(i)});$map_wrap.jmap('init',{'mapCenter':latlng},function(map,el,ops){$map_wrap.jmap('AddMarker',{'pointLatLng':ops.mapCenter,'pointHTML':$('.rslt-title',$listing).html()+$('.rslt-contact',$listing).html()});});setTimeout(function(){$map_wrap.jmap('CheckResize');},1000);}else if($this.attr('rel')=='reserve'){$('#rent_step1 form',$panel).rental_form();}});});}else{$panel.slideUp();$('.tab_link, .listing, .panel').removeClass('active');$('li','.tabs').removeClass('active');}
return false;});$('#get_dirs').live('submit',function(){var form=$(this),map_container=form.parents('#'+form.attr('data-container')),map_wrap=$('#'+form.attr('data-wrap'),map_container),from=$('#from_address',form).val(),to=form.attr('data-to-addr'),dirs=$('<div id="dirs" />');map_container.after(dirs);map_container.parent().prepend('<img src="http://s3.amazonaws.com/storagelocator/images/ui/storagelocator/usselfstoragelocator-sml.png" class="dp" />');map_wrap.jmap('SearchDirections',{'query':from+' to '+to,'panel':$('#dirs',map_container.parent()),'locale':'en_US'},function(){$('.ps',map_container.parent()).show();});return false;});var unit_size_form_partials={};$('.open_reserve_form').live('click',function(){var $this=$(this),rform=$('.reserve_form',$this.parent()),wrap=$this.parent('.sl-table-wrap'),listing_id=wrap.attr('rel').replace('listing_',''),size_id=wrap.attr('id').replace('Size_',''),accepts_reservations=wrap.attr('has-res')=='true'?true:false,ajax_loader=$('.ajax_loader',this);if(rform.hasClass('active')){rform.slideUp().removeClass('active');$('.sl-table').removeClass('active');}else{$('.reserve_form').slideUp().removeClass('active');$('.sl-table').removeClass('active');$('.sl-table',rform.parent()).addClass('active');if(unit_size_form_partials[size_id])rform.slideDown().addClass('active');else{ajax_loader.show();if(accepts_reservations){get_partial_and_do({partial:'views/partials/greyresults/reserve',model:'Listing',id:listing_id,sub_model:'Size',sub_id:size_id,show_size_ops:false},function(response){unit_size_form_partials[size_id]=response.data;rform.html(response.data).slideDown().addClass('active');ajax_loader.hide();$('#rent_step1 form',rform).rental_form();});}else{get_partial_and_do({partial:'views/partials/greyresults/request_info',model:'Listing',id:listing_id,sub_model:'Size',sub_id:size_id},function(response){unit_size_form_partials[size_id]=response.data;rform.html(response.data).slideDown().addClass('active');ajax_loader.hide();$('#rent_step1 form').rental_form();});}}}
$('input[type=text]:first',rform).focus();return false;});function bool_submit_once_and_do(form,wizard,slide_num,callback){var next_slide=$(wizard.slides[slide_num-1]),ajax_loader=$('.ajax_loader',next_slide);if(!form.data('submitted'))form.runValidation();if(!form.data('submitted')&&form.data('valid')){form.data('submitted',true).save_state();ajax_loader.show();submit_reservation_form(form,next_slide,ajax_loader,callback);return true;}else if(form.data('submitted')&&form.state_changed()){ajax_loader.show();if(slide_num==2){var step2=$('#reserve_step2',wizard.workflow).children().hide().end(),reservation_id=$('form',step2).attr('action').split('/');reservation_id=reservation_id[reservation_id.length-1];form.append('<input type="hidden" name="reservation_id" value="'+reservation_id+'" />');}else if(slide_num==3){}
form.save_state();submit_reservation_form(form,next_slide,ajax_loader,callback);return true;}else if(form.data('submitted'))return true;return false;}
function submit_reservation_form(form,next_slide,ajax_loader,callback){$.post(form.attr('action'),form.serialize(),function(response){$.with_json(response,function(data){callback.call(this,data,next_slide);},function(data){next_slide.html(data);});ajax_loader.hide();});}
$('form.new_listing_request').live('submit',function(){var form=$(this).runValidation(),ajax_loader=$('.ajax_loader',form).show();if(form.data('valid')&&!form.data('saving')){form.data('saving',true);$('.flash',form).slideUpRemove('slow');$.post(form.attr('action'),form.serialize(),function(response){if(response.success){var inner_panel=form.parent();inner_panel.children().fadeOut(300,function(){inner_panel.html(response.data).children().hide().fadeIn();$('.hintable',inner_panel).hinty();});}else form.prepend('<div class="flash flash-error">'+(typeof(response.data)=='object'?response.data.join('<br />'):response.data)+'</div>');ajax_loader.hide();form.data('saving',false);},'json');}else ajax_loader.hide();return false;});$('.tos').live('click',function(){get_pop_up_and_do({title:'Terms of Service',modal:true},{sub_partial:'pages/terms_of_service'});return false;});$('select.auto','#narrow_results_form').change(function(){delayed_submit(this);});$('.auto:not(select)','#narrow_results_form').click(function(){delayed_submit(this);});var feature_toggle=$('.openDiv','#unit_features');if(feature_toggle.length){feature_toggle.data('orig',feature_toggle.text());feature_toggle.click(function(){var $this=$(this);if(!$this.data('open')){$this.data('open',true);$this.text('Less Features');}else{$this.data('open',false);$this.text($this.data('orig'));}});}
function delayed_submit(input){setTimeout(function(){$(input).parents('form').submit();},100);}
function select_first_size_option(){$('li.rslt-price').each(function(){var option=$('.enabled:first',this);option.addClass('selected');$('input',option).attr('checked',true);});}
$.enableTooltips('a','.rslt-features');$('#narrow_results_form').submit(function(){var form=$(this).runValidation(),results_page=$('#ajax_wrap_inner'),results_wrap=$('#results_wrap',results_page),results_head=$('.rslt-head-txt',results_wrap),loading_txt='Looking for '+$('#search_storage_type',form).val()+' within <span class="hlght-text">'+
$('input[name="search[within]"]:checked',form).val()+'</span> miles of <span class="hlght-text">'+
$('#search_query',form).val()+'</span> '+$.ajax_loader_tag();$('#type-one-top-bar',results_wrap).fadeTo(500,.5);$('h2',results_head).removeClass('no_results hide').html(loading_txt);$('.txt_ldr',results_head).txt_loader();if(form.data('valid')&&!form.data('loading')){form.data('loading',true);$.getJSON(form.attr('action'),form.serialize(),function(response){$.with_json(response,function(data){results_page.replaceWith(data['results']);$.setGmap(data['maps_data']);$.enableTooltips('a','.rslt-features');select_first_size_option();$('.rslt-price','.listing').each(function(){$(':radio',this).eq(0).attr('checked',true);$('.radio_select',this).eq(0).addClass('checked');});});$('body').attr('id','listings_controller').addClass('locator_action');form.data('loading',false);});}
return false;});$('.list_sort').live('click',function(){var $this=$(this),form=$('#narrow_results_form','#content_bottom'),sort_fields=$('input.sort_field',form);if(!sort_fields.length){sort_fields='<div><input type="hidden" name="search[sorted_by]" class="sort_field" value="'+$this.attr('data-sorted_by')+'" />'+'<input type="hidden" name="search[sort_reverse]" class="sort_field" value="'+$this.attr('data-sort_reverse')+'" /></div>';form.append(sort_fields);}else{sort_fields.filter('input[name="search[sort_reverse]"]').val($this.attr('data-sort_reverse'));sort_fields.filter('input[name="search[sorted_by]"]').val($this.attr('data-sorted_by'));}
form.submit();return false;});var main_map=$('#main_map','#content');if(main_map.length){if(typeof Gmaps_data=='undefined'){var latlng=main_map.attr('data-latlng').split(',');$.setGmap({center:{lat:parseFloat(latlng[0]),lng:parseFloat(latlng[1]),zoom:14},maps:[]});$('#narrow_results_form').submit();}else $.setGmap({center:Gmaps_data.center,maps:Gmaps_data.maps});}
var featured_listing=$('#feat_wrap');if(featured_listing.length>0&&featured_listing.children().length==0&&$.on_page([['locator, home, show','listings, pages']])){get_partial_and_do({partial:'listings/featured'},function(response){$.with_json(response,function(partial){featured_listing.replaceWith(partial);});});}
var google_map=$('#google_map');if(google_map.length){var coords=$.map(google_map.attr('data-coords').split(','),function(el,i){return parseFloat(el)});google_map.jmap('init',{'mapCenter':coords},function(map,el,ops){google_map.jmap('AddMarker',{'pointLatLng':coords,'pointHTML':$('#fac_name span').text()});});}
var street_view=$('#street_view');if(street_view.length){var coords=$.map(google_map.attr('data-coords').split(','),function(el,i){return parseFloat(el)});street_view.jmap('init',{'mapType':G_HYBRID_MAP,'mapCenter':coords},function(map,el,ops){street_view.jmap('CreateStreetviewPanorama',{'latlng':coords});});}
$('#rentalizer').rental_form();if(window.location.hash=='#write_review')
get_review_pop_up({sub_partial:'comments/write_review',model:'Listing',id:$('listing_id').val()});$('a','#write_review').live('click',function(){var $this=$(this);$.new_ajax_loader('after',$this.parent()).show().fadeOutLater('fast',3000);get_review_pop_up({sub_partial:'comments/write_review',model:'Listing',id:$this.attr('data-listing_id')});return false;});$('#toggle_renting','#sl-edit-tabs').click(function(){var $this=$(this),ajax_loader=$.new_ajax_loader('after',this),toggle=$this.attr('data-toggle')=='true',toggle_html=$this.attr('data-toggle')=='true'?$('#toggle_renting_html','#tab2').html():'Are you sure you want to disable renting?';$.greyConfirm(toggle_html,function(){if(!$this.data('sending')){$this.data('sending',true);ajax_loader.show();$.post($this.attr('href'),function(response){$.with_json(response,function(data){$this.text((toggle?'Disable':'Enable')+' Renting').attr({'href':data.href,'data-toggle':(toggle?'false':'true')});if(toggle)
$('#listing_extra_form','#tab2').slideDown('slow',function(){$(this).removeClass('hide').addClass('enabled')});else
$('#listing_extra_form','#tab2').slideUp('slow',function(){$(this).removeClass('enabled').addClass('hide')});;});ajax_loader.hide();$this.data('sending',false)},'json');}});return false;});});function get_review_pop_up(options){get_pop_up_and_do({title:'Write a Review',width:500,modal:true},options,function(pop_up){$('#comment_name',pop_up).focus();$('form',pop_up).submit(function(){var form=$(this).runValidation(),ajax_loader=$.new_ajax_loader('after',$('input[type=submit]',this));if(form.data('valid')&&!form.data('sending')){ajax_loader.show();form.data('sending',true);$.post(form.attr('action'),form.serialize(),function(response){$.with_json(response,function(data){pop_up.html('<div class="framed" style="text-align:center;">'+data+'</div>');});ajax_loader.hide();form.data('sending',false);},'json');}
return false;});});}
...
return icon;}
try{var iconOptions={};iconOptions.width=32;iconOptions.height=32;iconOptions.primaryColor="#0000ff";iconOptions.cornerColor="#FFFFFF";iconOptions.strokeColor="#000000";var normalIcon=MapIconMaker.createMarkerIcon(iconOptions);var startIcon=new GIcon(G_DEFAULT_ICON,'http://s3.amazonaws.com/storagelocator/images/ui/map_marker.png');var normalIconImage=normalIcon.image,highlightIconImage='http://chart.apis.google.com/chart?cht=mm&chs=32x32&chco=FFFFFF,FBD745,000000&ext=.png',selectedIconImage='http://chart.apis.google.com/chart?cht=mm&chs=32x32&chco=FFFFFF,FB9517,000000&ext=.png';}catch(e){}
function highlightMarker(id){var marker=typeof id=='object'?id:getMarkerById(id);if(typeof(marker)!='undefined')marker.setImage('http://chart.apis.google.com/chart?chst=d_map_pin_letter&chld='+marker.mIndex+'|FED747|333333');}
function unhighlightMarker(id){var marker=typeof id=='object'?id:getMarkerById(id),def=typeof(marker)!='undefined';if(def&&marker.GmapState=='selected')marker.setImage('http://chart.apis.google.com/chart?chst=d_map_pin_letter&chld='+marker.mIndex+'|FED747|333333');else if(def)marker.setImage('http://chart.apis.google.com/chart?chst=d_map_pin_letter&chld='+marker.mIndex+'|339933|FFFFFF');}
function getMarkerById(id){var marker;$.each(GmapMarkers,function(){if(this.listing_id==id){marker=this;return;}});return marker;}
function addMarker(icon,lat,lng,title,body,bind_mouse_overs){if(typeof bind_mouse_overs=='undefined')var bind_mouse_overs=true;var point=new GLatLng(lat,lng);var marker=new GMarker(point,{'title':title,'icon':icon,width:'25px'});GEvent.addListener(marker,'click',function(){marker.openInfoWindowHtml(body);$('.listing').removeClass('active');$('#listing_'+marker.listing_id).addClass('active');});if(bind_mouse_overs){GEvent.addListener(marker,'mouseover',function(){$('.listing').removeClass('active');highlightMarker(marker);$('#listing_'+marker.listing_id).addClass('active');});GEvent.addListener(marker,'mouseout',function(){$('#listing_'+marker.listing_id).removeClass('active');unhighlightMarker(marker);});}
Gmap.addOverlay(marker);return marker;}
function make_indexed_icon(index){return new GIcon(G_DEFAULT_ICON,'http://chart.apis.google.com/chart?chst=d_map_pin_letter&chld='+index+'|339933|FFFFFF');}
GmapMarkers=[];$.setGmap=function(data,el){if(typeof el=='undefined')el='main_map';Gmap=new GMap2(document.getElementById(el));Gmap.addControl(new GLargeMapControl());Gmap.addControl(new GScaleControl());Gmap.addControl(new GMapTypeControl());Gmap.setCenter(new GLatLng(data.center.lat,data.center.lng),(data.center.zoom||16));Gmap.enableDoubleClickZoom();Gmap.disableContinuousZoom();Gmap.disableScrollWheelZoom();addMarker(startIcon,parseFloat(data.center.lat),parseFloat(data.center.lng),'Origin','<p><strong>Search distance measured from here.</strong></p>',false);var markers=data.maps;for(var i=0,len=markers.length;i<len;i++){var photo=markers[i].thumb?"<a href=\"/self-storage/show/"+markers[i].id+"#pictures\"><img style=\"margin-right:7px;border:1px solid #ccc;\" src="+markers[i].thumb+" width=\"80\" height=\"60\" align=\"left\" /></a>":'';var title=markers[i].title.replaceAll('+',' ');var body='<p>'+photo+'<span class="listing_title"><a href="/self-storage/show/'+markers[i].id+'">'+title+'</a></span>'+'<span class="listing_address">'+markers[i].address.replaceAll('+',' ')+'<br/>'+markers[i].city.replaceAll('+',' ')+', '+markers[i].state+' '+markers[i].zip+'</span>'+'</p>';var marker=addMarker(make_indexed_icon(i+1),markers[i].lat,markers[i].lng,title,body);marker.mIndex=i+1;marker.listing_id=markers[i].id;GmapMarkers[i]=marker;}
jQuery('.listing').live('mouseenter',function(){var id=$(this).attr('id').split('_')[1];highlightMarker(id);});jQuery('.listing').live('mouseleave',function(){var id=$(this).attr('id').split('_')[1];unhighlightMarker(id);});}
function update_info_tab_count(label,i){var tab=$('#tab_'+label,'#sl-tabs'),count=parseInt(tab.text().split('(')[1].replace(')',''))+i;tab.text(label.replace('_',' ')+' ('+count+')');}
$.clicked_on_different_tab=function($tab_link,$listing){var $open_panel=$('.panel:not(:hidden)','.listing');if($open_panel.length==0)return true;var clicked_listing=$open_panel.parent().attr('id'),active_listing=$listing.attr('id');if(active_listing!=clicked_listing)return true;var clicked_tab=$tab_link.attr('rel'),active_panel=$open_panel.attr('rel');return(clicked_tab!=active_panel&&active_listing==clicked_listing)||(clicked_tab==active_panel&&active_listing!=clicked_listing);}
$.activate_datepicker=function(context){$('.mini_calendar',context).datepicker();$('.datepicker_wrap',context).click(function(){$('.hasDatepicker',this).focus();});}
$.activateSizeSelect=function(context){var $size_picker=$('.size_picker',context),$size_img=$('img',$size_picker),$size_select=$('.sizes_select',context);var pre_loaded_size_icons=[];$('option',$size_select).each(function(){var $this=$(this);if($this.attr('data-url')!=''&&!$.any(pre_loaded_size_icons,function(){if(this.src==$this.attr('data-url'))return true;})){var img=new Image();img.src=$this.attr('data-url');pre_loaded_size_icons.push(img);}});if($size_select.length){size_icon_change($size_select);$size_select.live('change',size_icon_change);$('option',$size_select).live('mouseover',size_icon_change);}
function size_icon_change(input){var self=input[0]||this,$this=$(self),selected=self.tagName.toLowerCase()=='option'?$this.attr('data-url'):$('option:selected',self).attr('data-url'),new_img=$('<img src="'+selected+'" alt="" />'),size_details=capitalize(self.tagName.toLowerCase()=='option'?$this.attr('data-unit-type'):$('option:selected',self).attr('data-unit-type'));size_details+=" "+(self.tagName.toLowerCase()=='option'?$this.text():$('option:selected',self).text());if($size_img.attr('src')!=selected){$size_img.fadeOut(100,function(){$size_picker.html('').append(new_img).append('<p class="size_details">'+size_details+'</p>');new_img.hide().fadeIn(120);$size_img=$('img',$size_picker);if(new_img.width()>183)new_img.width(183);});}}}
$.enableTooltips=function(el,context,delay){$(el,context).tooltip({predelay:(delay||300)});}
$.calculatePrice=function(context){$.getJSON('/rentalizer',$.getCalculationParams(context),function(response){$.with_json(response,function(data){$.each(data,function(){var calc_wrap=$('#calcfor_'+this.listing_id),html='',paid_thru=new Date(this.calculation.paid_thru),months=paid_thru.getMonth()-new Date().getMonth();html+='<span class="price">$'+this.calculation.total+'</span><br />';html+='<span class="date">Paid for '+months+' month'+(months>1?'s':'')+'<br />thru '+paid_thru.format('longDate')+'</span>';calc_wrap.html(html);});});});}
$.getCalculationParams=function(context){var btns=$('.calc_params',context),params={multi_params:[]},now=new Date();btns.each(function(i){var b=$(this),p=[b.attr('data-listing-id'),b.attr('data-size-id'),b.attr('data-special-id')];params.multi_params.push(p.join('x'));});params.multi_params=params.multi_params.join('-');params.move_in_date=new Date(now.getYear(),now.getMonth(),now.getDate()+1).format('isoDate');return params;}
$.fn.special_txt_anim=function(){return this.each(function(){$(this).animate({'font-size':'120%'},150,function(){$(this).animate({'font-size':'100%'},300);});});}
$.fn.autoClickFew=function(max){if(typeof max=='undefined')var max=2;return this.each(function(i){if(i<=max)$(this).click();});}
$.fn.rental_form=function(){var rent_workflow={slides:[{div_id:'rent_step1',nav_vis:[['next',function(btn,wizard){btn.text('Next').fadeIn()}],['back','fadeOut']]},{div_id:'rent_step2',nav_vis:[['next',function(btn,wizard){btn.text('Rent It!').fadeIn()}],['back','fadeIn']],action:function(wizard){wizard.form_data=$('#rentalizer',wizard.workflow).serialize();$('.numeric_phone',wizard.workflow).formatPhoneNum();$('#new_tenant',wizard.workflow).unbind('submit').submit(function(){wizard.next();return false;});},validate:function(wizard){return $('#new_tenant',wizard.workflow).runValidation().data('valid');}},{div_id:'rent_step3',nav_vis:[['next','fadeOut'],['back','fadeOut']],action:function(wizard){var form=$('#new_tenant',wizard.workflow),ajax_loader=$('#processing_rental .ajax_loader',wizard.workflow).show();$('#processing_rental .flash',wizard.workflow).remove();wizard.form_data+='&'+form.serialize();$.post(form.attr('action'),wizard.form_data,function(response){$.with_json(response,function(data){$('#rental_complete',wizard.workflow).show();$('#processing_rental',wizard.workflow).hide();},function(data){$('#processing_rental',wizard.workflow).append('<div class="flash error">'+data.join('<br />')+'</div>');$('.back',wizard.nav_bar).show();});ajax_loader.hide();});}},],finish_action:function(wizard){wizard.workflow.parents('.panel').slideUp().removeClass('active').parents('.active').removeClass('active');}};function set_size_select(selects,unit_type,context){selects.hide();selects.filter('#size_id_'+unit_type,context).show();}
return this.each(function(){var form=$(this),special_btns=$('.avail_specials input',form),remove_special=$('.remove_special',form).hide(),type_select=$('select[name=unit_type]',form),unit_type=type_select.val().toLowerCase().replaceAll(' ','_').replaceAll('-','_').toLowerCase(),sizes_select=$('select.sizes_select',form),calendar=$('#move_in_date',form).datepicker({onSelect:function(date,ui){form.submit()},minDate:new Date()}),inputs={subtotal:$('.subtotal',form),multiplier:$('.multiplier',form),month_rate:$('.month_rate',form),paid_thru:$('.paid_thru',form),discount:$('.discount',form),usssl_discount:$('.usssl_discount',form),admin_fee:$('.admin_fee ',form),tax_amt:$('.tax_amt',form),total:$('.total',form)};form.submit(function(){$.getJSON(form.attr('action'),form.serialize(),function(response){$.with_json(response,function(data){for(key in data)
inputs[key].text(data[key]);});});return false;});new GreyWizard(form.parents('#rent_steps'),rent_workflow).begin_workflow_on(0);$.activateSizeSelect(form);$('.auto_next','#new_tenant').autoNext();$('#already_member','#new_tenant').click(function(){$.greyAlert('Sorry, this feature is not yet implemented.');return false;});if(!$('#size_ops',form).hasClass('hide')){sizes_select.change(function(){inputs.month_rate.text(sizes_select.children(':selected').attr('data-unit-price'));form.submit();});set_size_select(sizes_select,unit_type,form);type_select.change(function(){set_size_select(sizes_select,$(this).val().replaceAll(' ','_').replaceAll('-','_').toLowerCase(),form);});}
special_btns.click(function(){var $this=$(this);if($this.is(':checked')){remove_special.hide();$this.siblings('.remove_special').show();}
form.submit();});$('.calendar_wrap',form).click(function(){$(this).find('input').focus();});remove_special.click(function(){$(this).hide().siblings('input').attr('checked',false);form.submit();return false;});setTimeout(function(){if((s=special_btns.filter('.chosen')).length>0)
s.attr('checked',true).click();else
special_btns.eq(0).attr('checked',true).click();},100);});}
$=jQuery;$(function(){if($('body').hasClass('home'))$('#dock').jqDock({size:60,attenuation:400,fadeIn:1000});else $('#dock').jqDock({size:50,attenuation:400,fadeIn:1000});$.translate_with(translations);$('#search_submit, #search_submit2').click(function(){return $(this).parents('form').runValidation().data('valid');});$('#login_link').click(function(){var $this=$(this);if($this.hasClass('active'))return false;$this.addClass('active');var pop_up=$('#pop_up').dialog(default_pop_up_options({title:'Login to your account',width:347,height:'auto',modal:false,close:function(){$this.removeClass('active');}})).parent('.ui-dialog').css({top:'50px',right:'20px',left:'auto'});$('input[type=text]',pop_up).eq(0).focus();$.bindPlugins();return false;});$('#new_user_session','#pop_up.login_box').live('submit',function(){var form=$(this).runValidation(),overlay=$.applyLoadingOverlay(form.parents('#pop_up'));if(form.data('valid')&&!form.data('sending')){ajax_loader.show();overlay.fadeIn();form.data('sending',true);$.post(form.attr('action'),form.serialize(),function(response){$.with_json(response,function(data){var ready_mem=$('#ready_member',form);if(data.role=='advertiser'&&ready_mem.length==0){overlay.fadeOut('fast',function(){form.html('<p class="login_success">Looks good, redirecting to your account page.</p>')})}else{$('#topbar').html(data.html);$('#pop_up.login_box').fadeOutRemove();if(ready_mem.length>0){var values=ready_mem.val().split('|'),context=$('#'+values[0]),attributes=values[1].split(',')
field_ids=values[2].split(','),focus_el=values[3];$.each(field_ids,function(i){$('#'+this,context).val(data[attributes[i]]);});$('#'+focus_el,context).focus();$('#already_member',context).hide();}}},function(data){$('#pop_up.login_box').html(data);$('.fieldWithErrors input','#pop_up.login_box').eq(0).focus();});form.data('sending',false);ajax_loader.hide();},'json');}
return false;});$.applyLoadingOverlay=function(here){var overlay=$('<div class="overlay"><div></div></div>').appendTo(here).hide();return overlay;}
$('#forgot_pass_link','#pop_up').live('click',function(){var $this=$(this),pop_up=$('#pop_up.login_box'),ajax_loader=$.new_ajax_loader('after',this).show(),orig_html=pop_up.html();$this.hide();pop_up.load(this.href,function(response,status){$('input[type=text]',this).eq(0).focus();$.bindPlugins();$(this).dialog({title:'Enter your email',close:function(){pop_up.html(orig_html);$('#login_link').removeClass('active');$('.ajax_loader',pop_up).hide();$('#forgot_pass_link','#pop_up.login_box').show();}});ajax_loader.hide();});return false;});$('#password_resets_form').live('submit',function(){var form=$(this).runValidation(),ajax_loader=$('.ajax_loader',form);if(form.data('valid')){ajax_loader.show();$.post(form.attr('action'),form.serialize(),function(response){form.html(response);});}
return false;});$('#already_member').live('click',function(){$('#new_user_session').append('<input type="hidden" id="ready_member" value="'+$(this).attr('data-ready_member')+'" />');$('#login_link','#topbar').click();});var map_nav_btn=$('#map_nav_btn');if(map_nav_btn.length>0){$.preload_us_map_imgs();map_nav_btn.click(function(){var partial='menus/map_nav',title='Choose A State',height='486';get_pop_up_and_do({'title':title,'height':height,'modal':true},{'sub_partial':partial},function(){new GreyWizard($('#map_nav'),{title:'Choose a State',slides_class:'map_flow_step',nav_id:'map_flow_nav',slides:[{pop_up_title:'Click on a State',div_id:'map_step1',action:map_flow_step1,nav_vis:[['back','hide']]},{pop_up_title:'Pick a City',div_id:'map_step2',action:map_flow_step2,nav_vis:[['back','fadeIn']]}]}).begin_workflow_on(0);});return false;});function map_flow_step1(){var wizard=arguments[0],$map_img=$('#map_nav_img','#map_nav'),$areas=$('area','#USMap'),$state_name=$('#state_name','#map_nav');var add_map_overlay=function(){var area=$(this),img=$('<img class="map_overlay" src="http://s3.amazonaws.com/storagelocator/images/ui/storagelocator/us_map/'+area.attr('rel')+'.png" alt="" />');$state_name.text(area.attr('alt'));$map_img.before(img);};$areas.unbind('mouseenter',add_map_overlay).live('mouseenter',add_map_overlay);var remove_map_overlay=function(){$state_name.text('');$('.map_overlay','#map_nav').remove();};$areas.unbind('mouseleave',remove_map_overlay).live('mouseleave',remove_map_overlay);var get_cities=function(){var area=$(this),state=area.attr('alt');$state_name.text('Going to '+state+'...');if(state=='Washington DC'){window.location='/self-storage/washington-dc';}else{if(wizard.slide_data[1].data&&wizard.slide_data[1].data.state==state){wizard.slide_data[1].build_city_list=false;wizard.next();}else{$.getJSON('/ajax/get_cities?state='+state,function(response){$.with_json(response,function(data){wizard.slide_data[1].build_city_list=true;wizard.slide_data[1].pop_up_title='Pick a City in '+state
wizard.slide_data[1].data={state:state,cities:data};wizard.next();});});}}
return false;};$areas.unbind('click',get_cities).live('click',get_cities);}
function map_flow_step2(){var wizard=arguments[0];$('#city_name').remove();if(wizard.slide_data[1].build_city_list){var list=$('#cities_list','#map_step2').html(''),city_nav=$('#city_nav','#map_step2').html(''),city_link=$('<a href="/self-storage/"><span>Self Storage in </span></a>');for(var i=0,len=wizard.slide_data[1].data.cities.length;i<len;i++){var letter=wizard.slide_data[1].data.cities[i][0],cities=wizard.slide_data[1].data.cities[i][1],new_list=$('<li id="cities_'+letter+'" class="tab_content"></li>');city_nav.append('<li><a rel="cities_'+letter+'" href="#'+letter+'">'+letter+'</a></li>');for(var j=0,len2=cities.length;j<len2;j++){var new_city=city_link.clone(),city=cities[j];new_city.show().attr('href',city_link.attr('href')+wizard.slide_data[1].data.state+'/'+city.toLowerCase().replaceAll(' ','-'));new_city.find('span').hide().after(city);new_list.append(new_city);}
list.append(new_list);}
$('#map_step2').tabular_content();var city_click=function(){var city_name=$('#city_name','#map_step2');if(city_name.length>0)city_name.text('Looking for '+$(this).text()+', '+wizard.slide_data[1].data.state+'...')
...
$('p','#steps').hide();var $steps=$('.in','#steps'),fade_anim_speed=3400
$steps.hover(function(){fade_anim_step=$steps.index(this);$('img',$steps).fadeIn(600);$('p',$steps).fadeOut(600);$('img',this).fadeOut();$('p',this).fadeIn();},function(){$('p',this).fadeOut();$('img',this).fadeIn();});function stepsFadeAnim(){if(typeof(fade_anim_step)=='undefined'||fade_anim_step>=$steps.length-1)fade_anim_step=-1;fade_anim_step++;$('img',$steps).fadeIn(600);$('p',$steps).fadeOut(600);$('img',$steps.eq(fade_anim_step)).fadeOut(1200);$('p',$steps.eq(fade_anim_step)).fadeIn(1200,function(){setTimeout(function(){clearTimeout(fade_anim_int);fade_anim_int=setTimeout(stepsFadeAnim,fade_anim_speed);},fade_anim_speed);});}
$('#advanced_opts','#pages_controller.home').hide();$('.storage_in_city','#cities_list').css('width','23%');$('.storage_in_city span','#cities_list').hide();var aff_scroll=$('#aff_scroll');if(aff_scroll.length){$('.items',aff_scroll).width($('.usssl_adp',aff_scroll).length*160);aff_scroll.scrollable({speed:1500,circular:true,easing:'swing'});}
var tips_head=$('#tips-head');if(tips_head.length>0){new GreyShow({delay:5000,context:tips_head,slides:[{start:function(s){if($('.bubble, .purple_bgs',s.context).length<6){var tips_inner_html='<div class="purple_bgs" id="bg1"></div><div class="purple_bgs" id="bg2"></div><div class="purple_bgs" id="bg3"></div><div class="bubble" id="bub1"></div><div class="bubble" id="bub2"></div><div class="bubble" id="bub3"></div>';$('.bubble, .purple_bgs',s.context).remove();tips_head.append(tips_inner_html);}},objects:[{id:'bg3',action:'fadeIn',speed:500,delay:500},{id:'bub1',action:'fadeIn',speed:800,delay:5000,callback:function(o,s){o.html('<blockquote>I found it on USSelfStorageLocator.com</blockquote>').children().hide().fadeIn('slow')}}],end:function(s){s.gotoSlide(1);}},{objects:[{id:'bg1',action:'fadeIn',speed:500,delay:500},{id:'bub2',action:'fadeIn',speed:1000,delay:6000,callback:function(o,s){o.html('<blockquote>Online rentals are so convenient</blockquote>').children().hide().fadeIn('slow')}}],end:function(s){s.gotoSlide(2);}},{objects:[{id:'bg2',action:'fadeIn',speed:500,delay:500},{id:'bub3',action:'fadeIn',speed:1000,delay:8000,callback:function(o,s){o.html('<blockquote>I was able to find a really great deal!</blockquote>').children().hide().fadeIn('slow')}}],end:function(s){s.gotoSlide(0);}}]}).start();}
$('a[rel=sl-tabs-map]').click(function(){var map=$('#sl-tabs-map');if(!map.is(':hidden'))center_google_map();});$('.textFieldable','#SiteSettingFields .new_setting_field').live('mouseover',function(){var $this=$(this),settings_field_html='<input name="new_site_settings[][key]" value="'+$this.text()+'" class="hintable required" title="Enter a setting name" />';$(this).textFieldable(settings_field_html,function(text_field){$.revertSettingsTextFieldToLabel(text_field,$this.text());});});$('.textFieldable','#SiteSettingFields .existing_setting_field').live('click',function(){var $this=$(this),existing_settings_html='<input name="site_settings['+$this.text()+']" value="'+$this.text()+'" class="hintable required" title="Enter a setting name" />';$this.textFieldable(existing_settings_html,function(text_field){$.revertSettingsTextFieldToLabel(text_field,$this.text());});return false;});if($.on_page([['locator, home','listings']])){$('#top_map_btn').live('click',function(){var $this=$(this),main_map=$('#main_map'),location=$this.attr('data-loc').split(','),lat=parseFloat(location[0]),lng=parseFloat(location[1]);if($this.text()=='Show Map'){$.cookie('mo',true,{expires:30});$('span',$this).text('Hide Map');main_map.slideDown();}else{$.cookie('mo',null);$('span',$this).text('Show Map');main_map.slideUp();}
if(main_map.is(':visible'))setTimeout(function(){Gmap.checkResize();Gmap.setCenter(new GLatLng(lat,lng),12);},300);});if(!$.cookie('mo')){$.cookie('mo',true);$.open_map($('#main_map'));}}
if($.on_page([['new','permissions, roles']])){$('a.add_link','.partial_addable').click();}
$('a','#tips_sort').live('click',function(){$('a','#tips_sort').removeClass('up').removeClass('down');var $this=$(this),sort_what=$this.text(),tips=$('.blog-lock','#tips-wrap');switch(sort_what){case'Newest':$.sort_stuff($this,tips,'.share_wrap',function(a,b){var a1=parseInt($('.updated_at',a).text()),b1=parseInt($('.updated_at',b).text());return a1>b1?(stuff_sort_inverse?1:-1):(stuff_sort_inverse?-1:1);});break;case'Rating':$.sort_stuff($this,tips,'.share_wrap',function(a,b){var a1=$('.show-value',a).width(),b1=$('.show-value',b).width();return a1==b1?-1:a1>b1?(stuff_sort_inverse?-1:1):(stuff_sort_inverse?1:-1);});break;case'Title':$.sort_stuff($this,tips,'.share_wrap',function(a,b){var a1=$('h3 a',a).text(),b1=$('h3 a',b).text();return a1>b1?(stuff_sort_inverse?-1:1):(stuff_sort_inverse?1:-1);});break;}
return false;});$('input','#search_tips').keyup(function(){var parent=function(){return $(this).parents('.blog-lock')};$('.share_wrap > h3 a, .share_wrap > div','.blog-lock').search(this.value,'by substring',{remove:parent});});$('#create_tip').submit(function(){var form=$(this).runValidation(),ajax_loader=$.new_ajax_loader('after',$('input[type=submit]',form)).show();if(form.data('valid')&&!form.data('saving')){form.data('saving',true);$.post(form.attr('action'),form.serialize(),function(response){$.with_json(response,function(data){$('<div id="pop_up"><div id="tip_created">'+data+'</div></div>').dialog(default_pop_up_options({title:'Your Tip Was Submitted',height:'auto',width:'300px',modal:true}));form.parent().hide();});ajax_loader.hide();form.data('saving',false);},'json');}
return false;});var sharing_buttons=['email','facebook','twitter','digg','print'],addthis_main_btn='<a href="http://www.addthis.com/bookmark.php?v=250&username=mastermindxs" class="addthis_button_compact">Share</a><span class="addthis_separator">|</span>';$('.share_wrap').each(function(){if(typeof addthis=='object'){var share_wrap=$('.addthis_toolbox',this).append(addthis_main_btn),tip_link=$('.tip_link',this),share_url=tip_link.attr('href'),share_title=tip_link.text();$.each(sharing_buttons,function(){share_wrap.append('<a class="addthis_button_'+this+'"></a>');});addthis.toolbox(share_wrap[0],{'data_track_clickback':true},{url:share_url,title:share_title});}});$('.facebook_share','#tips-wrap').live('click',function(){var tip=$(this).parents('.share_wrap'),link=$('h3 a',tip),u=encodeURIComponent(link.attr('href')),t=encodeURIComponent(link.text());window.open('http://www.facebook.com/sharer.php?u='+u,'sharer','toolbar=0,status=0,width=626,height=436');return false;});$('#form_comments','#column_5').submit(function(){var form=$(this).runValidation(),ajax_loader=$.new_ajax_loader('before',$('input[type=submit]',form));if(form.data('valid')&&!form.data('saving')){form.data('saving',true);ajax_loader.show();$.post(form.attr('action'),form.serialize(),function(response){$.with_json(response,function(data){$.greyAlert(data,false);form[0].reset();});ajax_loader.hide();form.data('saving',false);},'json');}
return false;});$('form#new_client').submit(function(){if(!$('#chk_avail').hasClass('avail'))check_client_email_avail($('#client_email',this));var signup_form=$(this).runValidation();if(signup_form.data('valid')&&!signup_form.data('saving')){signup_form.data('saving',true);var pop_up_title='Add Your Facility',pop_up_height='auto',sub_partial='/clients/signup_steps',ajax_loader=$('#submit_wrap .ajax_loader',this).show(),current_step=1,form_data={company:$('#client_company',signup_form).val(),email:$('#client_email',signup_form).val(),city:$('#listing_city',signup_form).val(),state:$('#listing_state',signup_form).val()};$.post('/ajax/find_listings',form_data,function(response){$.with_json(response,function(data){get_pop_up_and_do({'title':pop_up_title,'height':pop_up_height,modal:true,width:'795px'},{'sub_partial':sub_partial},function(pop_up){var wizard=new GreyWizard($('#workflow_steps',pop_up),workflow_settings);if(data.length>0){workflow_settings.slides[0].data=data;wizard.begin_workflow_on(0);}else wizard.begin_workflow_on(1);signup_form.data('saving',false);});ajax_loader.hide();});},'json');}
return false;});$('#chk_avail').click(function(){return false;});$('#client_email','#new_client').blur(function(){check_client_email_avail($(this));});function check_client_email_avail(email_input){var form=$('#new_client').data('saving',true),chk_avail=$('#chk_avail',email_input.parent()).removeClass('avail').removeClass('not_avail'),email=email_input.val(),ajax_loader=$('.ajax_loader',email_input.parent());if(email==''||email==email_input.attr('title'))return false;if(!chk_avail.data('checking')){chk_avail.text('Checking').data('checking',true);$.getJSON('/ajax/find?model=Client&by=email&value='+email,function(response){$.with_json(response,function(data){if(data.length){email_input.addClass('invalid').focus();chk_avail.text('Already Taken').attr('title','You may have already signed up in the past. Try logging in.').removeClass('avail').addClass('not_avail').show();}else{email_input.removeClass('invalid');form.data('saving',false);chk_avail.text('Available').attr('title','Good to go!').removeClass('not_avail').addClass('avail');if(form_has_inputs_filled(form,['#listing_city','#listing_state']))
form.submit();}
chk_avail.data('checking',false);});ajax_loader.hide();});}}
function form_has_inputs_filled(form,input_ids){var all_filled=true;$.each(input_ids,function(){var input=$(eval("'"+this+"'"));if(input.val()==''||input.val()==input.attr('title'))all_filled=false;});return all_filled;}
if($.on_page([['edit','clients']])){$('.selective_hider').live('click',function(){var dont_hide=$(this).attr('rel'),hide_these=$('.hideable'),user_hints=$('.user_hint.open');user_hints.filter('.user_hint[data-rel!='+dont_hide+']').slideUp();user_hints.filter('.user_hint[data-rel='+dont_hide+']:hidden').slideDown();if(dont_hide){hide_these.each(function(){if(this.id!=dont_hide)$(this).slideUp();else $(this).slideDown();});}else{hide_these.slideDown();user_hints.slideDown();}
return false;});$('#reservations','#ov-services').click(function(){if($('#issn_enabled').val()!='false'){get_pop_up_and_do({title:'Reservations',height:'510',modal:true},{sub_partial:'clients/reservations',model:'Client',id:$('#client_id').text()},function(pop_up){pop_up.css('background-image','none');});}else{var partial='clients/issn_steps',title='Activate Real Time Reservations',height='auto';get_pop_up_and_do({title:title,height:height,modal:true},{sub_partial:partial,model:'Client',id:$('#client_id').text()},function(pop_up){new GreyWizard($('#issn_steps',pop_up),{title:title,slides:[{pop_up_title:title,div_id:'issnstep_1',nav_vis:[['back','hide'],['next',function(btn,wizard){btn.text('Next').data('done',false).fadeOut();}]],action:function(wizard){$('#issn_status_option a','#issnstep_1').unbind('click').click(function(){wizard.next();return false;});$('#slide_nav').remove();}},{pop_up_title:'Grant Access',div_id:'issnstep_2',nav_vis:[['back','fadeIn'],['next',function(btn,wizard){btn.text('Done').data('done',true).fadeIn();}]],action:function(wizard){if(typeof wizard.slide_data[1].client_info=='undefined'){$.getJSON('/ajax/get_partial?partial=clients/issn_agreement&model=Client&id='+$('#client_id').text(),function(response){$.with_json(response,function(data){wizard.slide_data[1].client_info=data;$('#client_info_preview',wizard.workflow).append(data);wizard.workflow.animate({height:'900px'});var pm_select=$('select[name=pm_software]',wizard.workflow);pm_select.change(function(){pm_select.removeClass('invalid');var span=pm_select.siblings('.val');if(!span.length)
span=pm_select.after('<span class="val dp"></span>').siblings('.val');span.text(pm_select.val());});$('.ps',wizard.workflow).click(function(){if(pm_select.val()==''){pm_select.addClass('invalid');return false;}});});});}else $('#client_info_preview',wizard.workflow).html(wizard.slide_data[1].client_info);}}],finish_action:'close'}).begin_workflow_on(0);});}
return false;});$('.pagination a, .table_sorter','#pop_up').live('click',function(){$('#pop_up').load(this.href+' #pop_up > div');return false;});$('.rsvr_detail_link','#client_reservations').live('click',function(){var $this=$(this),detail_box=$('.reservation_wrap',$this.parent()),ajax_loader=$('.ajax_loader',$this.parent()).show();if(detail_box.length==1){$('.reservation_wrap','td.region').hide();detail_box.show();ajax_loader.hide();}else{$.getJSON(this.href,{},function(response){$.with_json(response,function(data){var box=$(data).append('<a class="close_new_unit" href="#">X</a>');$('.reservation_wrap','td.region').hide();box.appendTo($this.parent());});ajax_loader.hide();});}
return false;});$('.close_btn','.reservation_wrap').live('click',function(){$(this).parent().hide();return false;});$('.inline_save').live('focus',function(){var input=$(this);input.after('<a class="submit_btn" href="#">Save</a>');});$('.attribute_save').live('click',function(){var save_btn=$(this),input=$('.inline_save',save_btn.parent());if(input.val()!=''&&input.val()!=inline_save_orig_values[input.attr('id')]){save_btn.attr('href',save_btn.attr('href')+input.value)}
$.log(this,inline_save_orig_values)
return false;});}
$('.hint_toggle').live('click',function(){var btn=$(this),hint=btn.parents('.user_hint'),placement_id=btn.parent('p').attr('id').replace('UserHintPlacement_',''),ajax_loader=$('.ajax_loader',hint).show();$.updateModel('/user_hints/'+placement_id+'/'+btn.attr('rel'),{model:'UserHintPlacement'},function(data){if(btn.attr('rel')=='hide'){hint.slideUp(900).removeClass('open').addClass('hide');hint.children().fadeOut(600);}else{hint.slideDown(900).removeClass('hide').addClass('open');hint.children().fadeIn(1200);}
ajax_loader.hide();});return false;});$('input','#user_hint_toggles').click(function(){$('.hint_toggle[rel='+this.value+']:'+(this.value=='open'?'hidden':'visible')).click();});$('#add_fac','#ov-units').click(function(){var $this=$(this),listing_box=$('#client_listing_box',$this.parent().parent()),ajax_loader=$this.prev('.ajax_loader').show();$.getJSON('/ajax/get_partial?model=Listing&partial=/listings/listing',function(response){$.with_json(response,function(data){var partial=$(data).hide(),title_input=$('input[name="listing[title]"]',partial),tip_text=$('.new_listing_tip',partial);if($('.listing',listing_box).length==0)listing_box.html('<div id="rslt-list-bg"></div>').find('#rslt-list-bg').append(partial);else $('#rslt-list-bg',listing_box).prepend(partial);$('.listing',listing_box).removeClass('active');partial.addClass('active').slideDown(300,function(){tip_text.fadeIn(600);title_input.focus();});bind_listing_input_events();$.bindPlugins();});ajax_loader.hide();});return false;});$('.cancel_link','#client_listing_box').live('click',function(){var $this=$(this),listing=$this.parents('.listing'),listing_id=listing.attr('id').replace('Listing_','');if(listing_id.length){$.greyConfirm('Are you sure?',function(){delete_client_listing(listing_id);listing.slideUpRemove();});}else listing.slideUpRemove();return false;});$('.delete_link','#client_listing_box').click(function(){var listing_id=$(this).parents('.listing').attr('id');delete_client_listing(listing_id);return false;});function delete_client_listing(listing_id){var ajax_loader=$('.ajax_loader','#ov-units-head').show();$.post('/clients/'+$('#client_id').text()+'/listings/'+listing_id.replace('Listing_','')+'/disable',{authenticity_token:$.get_auth_token()},function(response){$.with_json(response,function(data){$('#Listing'+listing_id,'#ov-units').slideUpRemove();});ajax_loader.hide();});}
$('.listing:eq(0) input[name="listing[title]"]','#client_listing_box').live('blur',function(){var partial=$('.listing:eq(0)','#client_listing_box'),title_input=$('input[name="listing[title]"]',partial).removeClass('invalid'),tip_text=$('.new_listing_tip',partial),tip_inner=tip_text.find('strong'),listing_id=partial.attr('id')?partial.attr('id').replace('Listing_',''):null;ajax_loader=$('#add_fac','#ov-units').prev('.ajax_loader').show();if(title_input.val()!=''&&title_input.val()!=title_input.attr('title')){tip_text.animate({top:'36px'});tip_inner.text('Enter the street address.');ajax_loader.show();var params={title:title_input.val(),client_id:$('#client_id').text()};if(listing_id)params['id']=listing_id;$.post('/listings/quick_create',params,function(response){if(response.success)partial.attr('id','Listing_'+response.data.listing_id);else title_input.addClass('invalid').focus();ajax_loader.hide();},'json');}else{title_input.focus();ajax_loader.hide();setTimeout(function(){title_input.addClass('invalid')},300);}});var listing_tip_inner_tag='strong',listing_input_msgs=[['address','Type in the city.',function(tip_text,msg){tip_text.animate({top:'60px'});tip_text.find(listing_tip_inner_tag).text(msg);}],['city','Enter the 2 letter State abbrev.',function(tip_text,msg){tip_text.find(listing_tip_inner_tag).text(msg);}],['state','Enter the 5 digit zip code.',function(tip_text,msg){tip_text.find(listing_tip_inner_tag).text(msg);}],['zip','<strong>Almost Done! Click Save.</strong>',function(tip_text,msg){tip_text.css('text-align','right').html('<strong>Almost Done! Click Save.</strong>');}]];function bind_listing_input_events(){$.each(listing_input_msgs,function(){var input_name=this[0],blur_msg=this[1],done_action=this[2],tip_text=$('.new_listing_tip','.listing:eq(0)');$('input[name="listing[map_attributes]['+input_name+']"]','.listing:eq(0)').live('blur',function(){var input=$('input[name="listing[map_attributes]['+input_name+']"]','.listing:eq(0)').removeClass('invalid');if(input.val()!=''&&input.val()!=input.attr('title'))done_action.call(this,tip_text,blur_msg);else input.focus().addClass('invalid');});});$('.action_btn a','.listing:eq(0)').live('click',function(){var partial=$('.listing:eq(0)','#client_listing_box'),button=$(this),ajax_loader=$('#add_fac','#ov-units').prev('.ajax_loader');if(!button.data('saving')&&button.text()=='Save'&&form_inputs_valid('.rslt_contact')){button.data('saving',true);ajax_loader.show();var listing_id=partial.attr('id').replace('Listing_',''),attributes={address:$('input[name="listing[map_attributes][address]"]',partial).val(),city:$('input[name="listing[map_attributes][city]"]',partial).val(),state:$('input[name="listing[map_attributes][state]"]',partial).val(),zip:$('input[name="listing[map_attributes][zip]"]',partial).val()};$.post('/listings/'+listing_id,{_method:'put',listing:{map_attributes:attributes},from:'quick_create',authenticity_token:$.get_auth_token()},function(response){$.with_json(response,function(data){button.text('Edit').unbind('click').attr('href','/clients/'+$('#client_id').text()+'/listings/'+listing_id+'/edit');listing=$(data);partial.html(listing.html()).removeClass('active');$('#listings_size').text(parseInt($('#listings_size').text())+1);});button.data('saving',false);ajax_loader.hide();},'json');return false;}});}
function form_inputs_valid(context){$('.i',context).each(function(){if($(this).hasClass('invalid'))return false;});return true;}
$('.edit_listing','#sl-edit-tabs').live('submit',function(){var form=$(this).runValidation(),context=form.parent(),btn=$('.save',form),ajax_loader=$('.ajax_loader',form);if(form.data('valid')&&!form.data('saving')){form.data('saving',true);ajax_loader.show();$.disable_submit_btn(btn);$.post(form.attr('action'),form.serialize(),function(response){$.with_json(response,function(data){if(data)$('#'+form.attr('data-target')).html(data);var success_msg=$('<span class="success_msg right">Saved!</span>');$('input[type=submit]',context).after(success_msg);success_msg.fadeOutLater('slow',3000);});$.activate_submit_btn(btn);ajax_loader.hide();form.data('saving',false);},'json');}
return false;});$('#tracking_num_req','#tab1').live('click',function(){var $this=$(this),ajax_loader=$.new_ajax_loader('after',$this).show();get_pop_up_and_do({title:'Request Tracked Number',modal:true},{sub_partial:'/listings/tracking_request',model:'Listing',id:$('#listing_id').val()},function(pop_up){$('.numeric_phone',pop_up).formatPhoneNum();});});$('.all_day_check','#hours').change(function(){var day_check=$(this),context=day_check.parent().parent().parent(),day_closed=$('.day_closed',context);day_check.data('was_checked',day_check.is(':checked'));if(day_check.is(':checked')){$('select, input[type=hidden]',context).attr('disabled',true);day_closed.each(function(){var check=$(this);check.data('was_checked',check.is(':checked'));});day_closed.attr('checked',true);$('select[rel=opening]',context).val('12:00 am');$('select[rel=closing]',context).val('12:00 am');}else{day_closed.each(function(){var check=$(this).attr('checked',false).change();if(check.data('was_checked')&&!check.is(':checked')){check.attr('checked',true);$('select, input[type=hidden]',check.parent()).attr('disabled',false);}});}});$('.all_day_check','#hours').each(function(){var $this=$(this);if($this.is(':checked'))$this.change();});$('.hour_range','#business_hours_form').each(function(){var fields=$('select, input[type=hidden]',this),checkbox=$(':checkbox',$(this).parent());if(!checkbox.is(':checked'))fields.attr('disabled',true);else fields.attr('disabled',false);});$('.day_closed','#business_hours_form').live('change',function(){var check=$(this),all_day_check=$('.all_day_check',check.parents('.hours_display'));check.data('was_checked',check.is(':checked'));if(check.is(':checked'))$('select, input[type=hidden]',$(this).parent().find('.hour_range')).attr('disabled',false);else $('select, input[type=hidden]',$(this).parent().find('.hour_range')).attr('disabled',true);if($('.day_closed:checked',check.parents('.hours_display')).length==1){all_day_check.data('was_checked',all_day_check.is(':checked'));all_day_check.attr('checked',false);}else if($('.day_closed:checked',check.parents('.hours_display')).length==0){all_day_check.attr('checked',all_day_check.data('was_checked'));}});$('.copy_all_hours','#business_hours_form').click(function(){var monday_range=$(this).parent(),checked=$(':checkbox',monday_range.parent()).is(':checked'),monday_hours=$('select',monday_range),other_hours=$('.hour_range select',monday_range.parent().parent()).not(monday_hours);other_hours.each(function(){var this_hour=$(this),this_day=this_hour.parent().parent();if(this_hour.attr('rel')=='opening')this_hour.val(monday_hours.eq(0).val());else if(this_hour.attr('rel')=='closing')this_hour.val(monday_hours.eq(1).val());$(':checkbox',this_day).attr('checked',checked);$('select, input[type=hidden]',this_day).attr('disabled',!checked);});return false;});$('#save_hours','#hours').click(function(){var $this=$(this),form=$this.parents('#hours').find('form'),ajax_loader=$('.ajax_loader','#hours');if(!form.data('saving')){form.data('saving',true);ajax_loader.show();$.disable_submit_btn($this);$.post(form.attr('action'),form.serialize(),function(response){$.with_json(response,function(data){$this.after('<span class="success_msg">Saved!</span>');setTimeout(function(){$('.success_msg','#hours').fadeOutRemove(1000);},3000);});$.activate_submit_btn($this);form.data('saving',false);ajax_loader.hide();},'json');}
return false;});$('#sync_listing').click(function(){var $this=$(this).text('Syncing'),ajax_loader=$this.siblings('.ajax_loader').show(),sizes_in=$('#sl-tabs-sizes-in').addClass('faded');$.post($this.attr('href'),{},function(){$.greyAlert('Your unit details are being synced. Reload the page after few minutes to see the changes.',false);},'json');return false;});$('#picture_facility_image','#new_picture').live('change',function(){var thumb=$('<li><img src="http://s3.amazonaws.com/storagelocator/images/ui/ajax-loader-lrg.gif" class="loading" alt="" /><a class="iconOnly16 delete_link right" title="Delete this picture">Delete</a></li>');;if($('.main_pic','#sl-tabs-pict-in').length==0){var image=$('<img class="big-pic" src="" alt="" />');$('.gallery','#sl-tabs-pict-in').append(image);}
if($(this).val()!='')$('#new_picture').ajaxSubmit({dataType:'json',beforeSubmit:function(arr,$form,options){$('#sl-tabs-pict-gall').append(thumb);thumb.hide().fadeIn(600);setTimeout(function(){$('#picture_facility_image',$form).val('')},100);},success:function(response){$.with_json(response,function(data){var thumb_img=$('img',thumb);thumb_img.attr({src:data.thumb,id:'Picture_'+data.id}).removeClass('loading');thumb_img.after('<a href="/listings/'+data.listing_id+'/pictures/'+data.id+'" class="delete_link right">Delete</a>');if(image)image.attr('src',data.image);thumb_img.trigger('mouseover');});}});});$('#client_edit_contact').live('click',function(){var $this=$(this);$.authenticate_user_and_do($this,function(data){var cancel_link=$('<a class="cancel_link iconOnly16 right" style="margin-top:13px;" title="Cancel Editing">Cancel</a>'),wrap=$('#owner_info_wrap',$this.parent().parent()),ajax_loader=$.new_ajax_loader('before',$this);if($this.text()=='Edit'){ajax_loader.show();$.getJSON($this.attr('href'),function(response){$.with_json(response,function(data){$this.text('Save').after(cancel_link);wrap.hide().after(data);$('.numeric_phone',wrap.parent()).formatPhoneNum();$('.hintable',wrap.parent()).hinty();$('.auto_next',wrap.parent()).autoNext();});ajax_loader.hide();});}else if($this.text()=='Save'){var form=$('#edit_info',wrap.parent()).runValidation();if(!$this.data('saving')&&form.data('valid')){$this.data('saving',true);$('.cancel_link',$this.parent()).remove();ajax_loader.show();$.post(form.attr('action'),form.serialize(),function(response){$.with_json(response,function(data){$this.text('Edit').after('<span class="success_msg">Saved!</span>').next('.success_msg').fadeOutLater('slow',3000);wrap.show().html(data);form.remove();});$this.data('saving',false);ajax_loader.hide();});}}
cancel_link.click(function(){$('#edit_info').remove();$('#owner_info_wrap').show();$(this).fadeOutRemove(300);$('#client_edit_contact').text('Edit');return false;});},$this.text()=='Save');return false;});var fac_photo_show=$('#sl-photos');if(fac_photo_show.length>0){var imgs=$('img:not(.main_pic)',fac_photo_show),count=0;$.setInterval(function(){var index=count%imgs.length;$(imgs[index]).trigger('mouseover');count++;},7000);}
$('#sl-tabs-pict-gall img, #previews img').live('mouseover',function(){if($(this).hasClass('loading'))return false;var main_pic=$('#sl-tabs-pict .main_pic, #sl-photos .main_pic');if(main_pic.length==0)main_pic=$('<img class="main_pic hide" src="'+this.src.replace('/thumb_','/medium_')+'" alt="'+this.alt+'" />').appendTo('#main_pic_wrap');else if(main_pic.length>1){var p=main_pic.eq(0);main_pic.not(p).remove();main_pic=p;}
$('img','#sl-photos #previews, #sl-tabs-pict-gall').removeClass('active');var thumb=$(this),new_img=$('<img class="main_pic hide" src="'+thumb.attr('src').replace('/thumb_','/medium_')+'" alt="'+thumb.attr('alt')+'" />');new_img.load(function(){thumb.addClass('active');main_pic.after(new_img).fadeOutRemove(900);new_img.fadeIn(900);});}).live('click',function(){return false});$('.delete_link','#sl-tabs-pict').live('click',function(){var $this=$(this);if(!$this.data('deleting'))$.greyConfirm('Are you sure you want to delete this picture?',function(){$this.data('deleting',true).text('Deleting...');var img=$this.prev('img'),id=img.attr('id').replace('Picture_','');$.post($this.attr('href'),{_method:'delete',authenticity_token:$.get_auth_token()},function(response){$.with_json(response,function(data){if(img.hasClass('active'))
$('img:not(#'+img.attr('id')+')','#sl-tabs-pict').trigger('mouseover');if($('img','#sl-tabs-pict-gall').length==1)
$('.main_pic','#sl-tabs-pict').eq(0).fadeOutRemove(900);$this.parent().fadeOutRemove(600);});},'json');});return false;});$('#account_home_link').click(function(){$('#stats_graph').css('width','700px');init_stats_graph();});$('.auto_change','#ov-reports-cnt').change(function(){$('#stats_graph').children().fadeTo('slow',.5);init_stats_graph({months_ago:this.value,force:true});});function init_stats_graph(options){if(typeof options=='undefined')var options={}
var stats_graph=$('#stats_graph'),days_ago=options.days_ago||0,months_ago=options.months_ago||1,years_ago=options.years_ago||0,force=options.force||false;if(stats_graph.length>0){stats_graph.addClass('loading');var issn_enabled=$('input#issn_enabled').val()=='false'?false:true,stats_models='clicks,impressions,'+(issn_enabled?'rentals':'info_requests'),d=new Date(),end_date=new Date(d.getFullYear(),d.getMonth(),d.getDate()+1),start_date=new Date((d.getFullYear()-years_ago),(d.getMonth()-months_ago),(d.getDate()-days_ago));$.getJSON('/ajax/get_client_stats?start_date='+start_date+'&end_date='+end_date+'&stats_models='+stats_models+'&client_id='+$('#client_id').text(),function(response){$.with_json(response,function(data){$.jqplot.preInitHooks.push(function(){stats_graph.children().remove();});var plot_data=[],stats_arr=stats_models.split(/,\W?/);for(i in stats_arr)
plot_data.push(data['data'][stats_arr[i]]);$.jqplot('stats_graph',plot_data,{axes:{xaxis:{renderer:$.jqplot.DateAxisRenderer,rendererOptions:{tickRenderer:$.jqplot.CanvasAxisTickRenderer},tickOptions:{formatString:'%b %#d, %Y',fontSize:'12px'}},yaxis:{min:0,max:parseInt(data['max'])+1},},legend:{show:true,location:'nw',xoffset:10,yoffset:10},series:[{label:' Clicks',lineWidth:2,color:'#3333CC',markerOptions:{style:'diamond',color:'#3333CC'}},{label:' Impressions',lineWidth:2,color:'#FED747',markerOptions:{size:7,style:'circle',color:'#FED747'}},{label:' '+(issn_enabled?'Rentals':'Requests'),lineWidth:2,color:'#339933',markerOptions:{style:'circle',color:'#339933'}}],highlighter:{sizeAdjust:7.5},cursor:{show:true,zoom:true,followMouse:true,tooltipLocation:'ne'},grid:{background:'#ffffff'}});});stats_graph.removeClass('loading');});}}
init_stats_graph({months_ago:$('select.auto_change','#ov-reports-cnt').val()});$('.client_tip:not(:first)','#tips-box').hide();var client_tip_boxes=$('.client_tip','#tips-box');if(client_tip_boxes.length>0){$('a','#tips-box-bottom').click(function(){if(client_tip_boxes.length>1){var $this=$(this),direction=$this.attr('id')=='next_tip'?1:-1,current_tip=$('.client_tip:visible','#tips-box'),current_index=client_tip_boxes.index(current_tip),new_index=current_index+direction;if(new_index==client_tip_boxes.length)new_index=0;else if(new_index<0)new_index=client_tip_boxes.length-1;current_tip.hide();$(client_tip_boxes[new_index]).show();}
return false;});}
$('.save_btn').click(function(){var $this=$(this),form=$('form',$this.attr('context')).runValidation(),ajax_loader=$($this.attr('loader'));if(form.data('valid')){$.authenticate_user_and_do($this,function(data){if(!form.data('saving')){form.data('saving',true);$this.text('Updating');ajax_loader.show();$.post(form.attr('action'),form.serialize(),function(response){$.with_json(response,function(data){$($this.attr('replace'),$this.attr('context')).replaceWith(data);$this.after('<span class="success_msg">Saved!</span>').next('.success_msg').fadeOutLater('slow',3000);});form.data('saving',false);$this.text('Update');ajax_loader.hide();});}});}
return false;});$('#logo_form').live('submit',function(){var form=$(this),btn=$('.save',form);form.ajaxSubmit({beforeSubmit:function(){$.disable_submit_btn(btn);$('.ajax_loader','#logo_form').show();},target:'#flogo'});return false;});$('.default_logo','#logo_choices').live('click',function(){var img=$(this),index=img.attr('data-ci');img.attr('src','http://s3.amazonaws.com/storagelocator/images/ui/ajax-loader-lrg.gif').css({'height':'44px','border-color':'#fff'});$.post('/clients/'+$('#client_id').val()+'/listings/'+$('#listing_id').val(),{authenticity_token:$.get_auth_token(),from:'uplogo',default_logo:index,_method:'put'},function(response){$('#flogo','#tab1').html(response);});});$('.open_small_partial').live('click',function(){var $this=$(this);get_pop_up_and_do({title:$this.attr('title'),width:400,height:248},{sub_partial:$this.attr('href')},function(pop_up){pop_up.css('width','400px')});return false;});$('.popup-post').live('click',function(){var $this=$(this);$.getJSON(this.href,function(response){$.with_json(response,function(data){var pop_up=$('<div id="pop_up"></div>');pop_up.html(data).dialog(default_pop_up_options({title:$this.attr('title'),width:'400px'}));$('')});});return false;});$('.ps').live('click',function(){var div_to_print=this.rel,print_opts={operaSupport:$.browser.opera};if($(this).attr('href')=='#')$(div_to_print).jqprint(print_opts);else{$.getJSON(this.href,function(response){$.with_json(response,function(data){var wrap=$(data).appendTo('body'),coup=$(div_to_print).clone().appendTo('#print_content');wrap.jqprint(print_opts,function(){wrap.remove();});});});}
return false;});$('#resend_link','#signupstep_5').live('click',function(){var $this=$(this),ajax_loader=$.new_ajax_loader('after',$this.parent());if(!$this.data('sending')){$this.data('sending',true);ajax_loader.show();$.getJSON($this.attr('href'),function(response){$.with_json(response,function(data){$this.parent().after('<p class="success_msg">The activation email has been resent.</p>');});ajax_loader.hide();$this.data('sending',false);});}
return false;});});var workflow_settings={title:'Add Your Facility',nav_id:'workflow_nav',set_slides:false,width:753,slides:[{div_id:'signupstep_2',action:workflow_step2,nav_vis:[['next',function(btn,wizard){btn.text('Next').data('done',false).show()}],['skip',function(btn,wizard){btn.fadeIn().bind('click',ensure_no_listings_checked)}],['back',function(btn,wizard){btn.show().bind('click',close_pop_up_and_focus_on_fac_name)}]]},{div_id:'signupstep_3',action:workflow_step3,nav_vis:[['next',function(btn,wizard){btn.text('Next').data('done',false).show()}],['skip','fadeOut'],['back',function(btn,wizard){btn.unbind('click',close_pop_up_and_focus_on_fac_name)}]],validate:function(wizard){return $('#contact_info_form',wizard.workflow).runValidation().data('valid');}},{div_id:'signupstep_4',action:workflow_step4,nav_vis:[['next',function(btn,wizard){btn.text('Submit').data('done',false);}],['skip','hide'],['back','fadeIn']],validate:function(wizard){return $('#terms_use',wizard.workflow).runValidation().data('valid');}},{div_id:'signupstep_5',action:workflow_step5,nav_vis:[['next',function(btn,wizard){btn.text('Done').data('done',true);}],['skip','hide'],['back','fadeIn']]}],finish_action:function(wizard){wizard.workflow.parent().dialog('destroy').remove();$('#new_client')[0].reset();$('#chk_avail, .ajax_loader','#new_client').hide();}};function ensure_no_listings_checked(){$('input[name=listing_id]:checked','#signupstep_2').attr('checked',false).parents('.selected').removeClass('selected');}
function close_pop_up_and_focus_on_fac_name(event){$('#pop_up').dialog('close');$('#client_company','#new_client').focus();}
function workflow_step2(wizard){var listings_box=$('.small_listings',arguments[0].workflow);if(listings_box.children().length==0){listings_box.hide();var listing_prototype=$('.listing_div',arguments[0].workflow).eq(0).removeClass('hidden').remove();$('.found_box p span',wizard.workflow).text(wizard.slide_data[0].data.length);$.each(wizard.slide_data[0].data,function(i){var listing=this.listing,listing_div=listing_prototype.clone();$('.check input',listing_div).val(listing.id);$('.num',listing_div).text(i+1);$('.listing_title',listing_div).text(listing.title);$('.listing_address',listing_div).html('<span class="street_address">'+listing.address+'</span><br />'+listing.city+', '+listing.state+' <span class="zip">'+listing.zip+'</span>');listing_div.attr('id','Listing_'+listing.id).appendTo(listings_box);});setTimeout(function(){listings_box.fadeIn(wizard.settings.fade_speed);var listing_id=$.get_param_value('listing_id');if(listing_id)
$('#Listing_'+listing_id,listings_box).addClass('selected').find(':checkbox[name=listing_id]').attr('checked',true);},350);}}
function workflow_step3(wizard){var addresses=$.get_checked_listings_addresses(wizard),city=$('#listing_city','#new_client').val(),state=$('#listing_state','#new_client').val(),zips=$.get_checked_listings_addresses(wizard,'zip');$.setup_autocomplete('#listing_city',wizard.workflow);if(addresses.length==1)$('#listing_address',wizard.workflow).val(capitalize_addr(addresses[0]));else if(addresses.length>1)$('#listing_address',wizard.workflow).autocomplete({source:capitalize_addr(addresses||[])});if(zips.length==1)$('#listing_zip',wizard.workflow).val(zips[0]);else if(zips.length>1)$('#listing_zip',wizard.workflow).autocomplete({source:zips});$('#listing_city',wizard.workflow).val(capitalize(city));$('#listing_state',wizard.workflow).val(state.toUpperCase());$('.hintable',wizard.workflow).hinty();$('.numeric_phone',wizard.workflow).formatPhoneNum();$('.city_state_zip .autocomplete',wizard.workflow).autocomplete();setTimeout(function(){$('#first_name',wizard.workflow).focus()},350);}
function workflow_step4(){var wizard=arguments[0],review=$('#signupstep_4 .inner',wizard.workflow).html(''),listings=$('#signupstep_2 .small_listings',wizard.workflow).find('input:checked'),info=$('#signupstep_3 #contact_info_form',wizard.workflow).find('input'),company=$('#client_company','#new_client').val(),email=$('#client_email','#new_client').val();wizard.form_data.client={};wizard.form_data.mailing_address={};wizard.form_data.client['company']=company;wizard.form_data.client['email']=email;wizard.form_data['authenticity_token']=$.get_auth_token();info.each(function(){switch(this.name){case'first_name':wizard.form_data.client['first_name']=capitalize(this.value);break;case'last_name':wizard.form_data.client['last_name']=capitalize(this.value);break;case'listing_address':wizard.form_data.mailing_address['address']=this.value;break;case'listing_city':wizard.form_data.mailing_address['city']=capitalize(this.value);break;case'listing_state':wizard.form_data.mailing_address['state']=this.value;break;case'listing_zip':wizard.form_data.mailing_address['zip']=this.value;break;case'listing_phone':wizard.form_data.mailing_address['phone']=this.value||'';break;case'wants_newsletter':wizard.form_data.client[this.name]=this.checked;break;}});var review_html='<h4>Contact Information:</h4>';review_html+='<div id="review_contact">';review_html+='<div class="label">Company Name:</div> <p class="listing_title">'+titleize(company)+'</p>';review_html+='<div id="address" class="label">Company Address:</div> <p class="listing_address">'+
...
wizard.form_data.mailing_address['zip']+'</p>';review_html+='<div id="name" class="label">Name:</div> <p class="name">'+wizard.form_data.client['first_name']+' '+wizard.form_data.client['last_name']+'</p>';if(wizard.form_data.mailing_address['phone']&&wizard.form_data.mailing_address['phone']!='Phone Number')
review_html+='<div class="label">Phone:</div> <p class="phone">'+wizard.form_data.mailing_address['phone']+'</p>';review_html+='<div class="label">Email:</div> <p class="email">'+email+'</p>';review_html+='</div>';review_html+=(wizard.form_data.client['wants_newsletter']?'<p class="opt_in">Send':'<p class="opt_out">Don\'t send')+' me the monthly newsletter.</p>';if(listings.length>0){$('.attribute_fields .listing,.attribute_fields .map',review.next()).remove();wizard.form_data.listings=[];review_html+='<h4 id="listings">My Listings:</h4><div class="small_listings">';listings.each(function(i){var title=titleize($('#Listing_'+this.value+' .listing_title',wizard.workflow).text()),address=titleize($('#Listing_'+this.value+' .listing_address',wizard.workflow).html());wizard.form_data.listings.push(this.value);review_html+='<div class="listing_div"><div class="left block num">'+(i+1)+'</div><div class="listing_in">';review_html+='<p class="listing_title">'+title+'</p><p class="listing_address">'+address+'</p></div></div>';});review_html+='</div>';}
review.append(review_html);setTimeout(function(){review.fadeIn(wizard.settings.fade_speed)},350);}
function workflow_step5(wizard){var nav_btns=$('.button',wizard.nav_bar).hide();$('#signup_processing .ajax_loader',wizard.workflow).fadeIn();$.post('/clients',wizard.form_data,function(response){$.with_json(response,function(data){nav_btns.filter('.next').show();$('#signup_processing',wizard.workflow).hide();$('#signup_complete',wizard.workflow).show();$('#resend_link',wizard.workflow).attr('href','/resend_activation/'+data.activation_code);});},'json');}
$.get_checked_listings_addresses=function(wizard,address_part){if(typeof address_part=='undefined')var address_part='street_address';var checked=$('#signupstep_2 :checkbox:checked',wizard.workflow),addresses=[];checked.each(function(){var part=$('.'+address_part,'#Listing_'+this.value).text();addresses.push(part);});return addresses;}
$.preload_us_map_imgs=function(){var states=["al","ak","az","ar","ca","co","ct","de","fl","ga","hi","id","il","in","ia","ks","ky","la","me","md","ma","mi","mn","ms","mo","mt","ne","nv","nh","nj","nm","ny","nc","nd","oh","ok","or","pa","ri","sc","sd","tn","tx","ut","vt","va","wa","wv","wi","wy"];$.each(states,function(){var img=new Image();img.src='http://s3.amazonaws.com/storagelocator/images/ui/storagelocator/us_map/'+this+'.png';});}
var translations=[{page:'.spanish',elements:[{element:'.virtual_form #comment_submit',method:'val',translation:'Enviar Mensaje'},{element:'#footer_top .wrapper',method:'text',translation:'Encuentre un espacio de almacenamiento en cualquier lugar y en cualquier momento!'},{element:'#title h1',method:'text',translation:'Busque, Ahorre y Rente Almacenamiento En Cualquier Momento.'},{element:'#aff-box p',method:'html',translation:'<strong>Socios Afiliados</strong> - Obtenga grandes ofertas y ahorre en su próximo alquiler de almacenamiento con uno de nuestros socios afiliados.'},{element:'#social-media p',method:'html',translation:'<strong>Compartir esta pagina</strong> - Tenemos ofertas increíbles a través de nuestras páginas de la red social y empresarial.'},{element:'#top_cities p strong',method:'text',translation:'Las Ciudades Mas Populares De Almacenamiento'}]}]
$.translate_with=function(translations){$.each(translations,function(){var page=this.page;$.each(this.elements,function(){var element=$(this.element,page);if(element.length>0)
