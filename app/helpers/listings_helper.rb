module ListingsHelper
  ...
  def more_results_link(data)
    ...
    if range_start < data.total_entries - per_page+1
      ...
      html << "<span class='hidden' name='params_query'>#{(params[:search][:query] rescue nil) || params[:city]}</span>"
      html << "<span class='hidden' name='params_page'>#{((params[:page] ? params[:page].to_i : 1) + 1)}</span>"
      html << "<span class='hidden' name='params_within'>#{(params[:search][:within] rescue nil)}</span>"
    end
