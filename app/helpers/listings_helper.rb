module ListingsHelper
  ...
  def edit_listing_city_state_zip(listing)
    if listing.nil? || listing.new_record?
      ...
      html << text_field_tag('listing[map_attributes][state]', nil, :class => 'required autocomplete hintable tiny_text_field i', :title => 'State', :'data-autocomp-source' => 'UsCitys_abbrevs', :maxlength => 2)
  ...
  def claim_listing_link(listing, options = {})
    if listing.client.nil? || listing.client.status == 'unverified'
      link_to 'Claim & Verify Ownership', "#{request.protocol}#{request.host}/self-storage-advertising?client[company]=#{CGI.escape listing.title}&listing[city]=#{CGI.escape listing.city}&listing[state]=#{CGI.escape listing.state}&listing_id=#{listing.id}", options.merge(:title => 'Claim this listing if you are the verifiable owner/manager.')
    end
  end
